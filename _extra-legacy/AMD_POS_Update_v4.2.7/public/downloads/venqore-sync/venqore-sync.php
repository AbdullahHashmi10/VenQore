...
define('VENQORE_SYNC_VERSION', '1.1.0');
...
function venqore_sync_init() {
    ...
    // Custom product list columns styles
    add_action('admin_head', 'venqore_sync_admin_table_styles');
}

/**
 * Custom styling for the VenQore Sync column in the products table to prevent text squishing.
 */
function venqore_sync_admin_table_styles() {
    $screen = get_current_screen();
    if ($screen && $screen->id === 'edit-product') {
        echo '<style>
            .fixed .column-venqore_sync { width: 130px !important; }
            th.column-venqore_sync { white-space: nowrap !important; min-width: 110px !important; }
            td.column-venqore_sync { vertical-align: middle !important; white-space: nowrap !important; }
        </style>';
    }
}
...
// ─── Native WordPress Plugin Auto-Update Checker ──────────────────────────────

add_action('admin_init', 'venqore_sync_init_update_checker');

function venqore_sync_init_update_checker() {
    // Only run update check in admin panel and if connected
    if (!is_admin()) {
        return;
    }

    $connected = get_option('venqore_sync_connected');
    $conn_info = get_option('venqore_sync_info', []);
    $site_url  = isset($conn_info['api_url']) ? $conn_info['api_url'] : get_option('venqore_site_url', '');

    if (!$connected || empty($site_url)) {
        return;
    }

    // Hook into update transient check
    add_filter('pre_set_site_transient_update_plugins', 'venqore_sync_check_plugin_update');
    // Hook into plugin information popup screen
    add_filter('plugins_api', 'venqore_sync_plugin_update_info', 20, 3);
}

/**
 * Filter transient to inject plugin updates from our POS server
 */
function venqore_sync_check_plugin_update($transient) {
    if (empty($transient->checked)) {
        return $transient;
    }

    $plugin_slug = 'venqore-sync/venqore-sync.php';
    $current_version = isset($transient->checked[$plugin_slug]) ? $transient->checked[$plugin_slug] : VENQORE_SYNC_VERSION;

    // Strict 12-hour transient cache to prevent admin performance bottlenecks
    $cached = get_transient('venqore_sync_update_check_cache');
    if ($cached !== false) {
        if (!empty($cached['new_version']) && version_compare($cached['new_version'], $current_version, '>')) {
            $transient->response[$plugin_slug] = (object) $cached;
        }
        return $transient;
    }
    ...
    // Call our public check-update API route on POS
    $update_url = esc_url_raw(add_query_arg('version', $current_version, trailingslashit($site_url) . 'api/woo/plugin/check-update'));
    ...
    if (is_wp_error($response) || wp_remote_retrieve_response_code($response) !== 200) {
        // Cache empty result for 1 hour to prevent flooding the server on failures
        set_transient('venqore_sync_update_check_cache', [], HOUR_IN_SECONDS);
        return $transient;
    }

    $body = json_decode(wp_remote_retrieve_body($response), true);
    if (!empty($body) && !empty($body['new_version'])) {
        $update_data = [
            'slug'        => $body['slug'],
            'plugin'      => $body['plugin'],
        ...
        set_transient('venqore_sync_update_check_cache', $update_data, 12 * HOUR_IN_SECONDS);

        if (version_compare($body['new_version'], $current_version, '>')) {
            $transient->response[$plugin_slug] = (object) $update_data;
        }
    } else {
        set_transient('venqore_sync_update_check_cache', [], 12 * HOUR_IN_SECONDS);
    }
...
/**
 * Hook to render details overlay modal when user clicks "View details"
 */
function venqore_sync_plugin_update_info($result, $action, $args) {
    if ($action !== 'plugin_information' || $args->slug !== 'venqore-sync') {
        return $result;
    }
    ...
    $update_url = esc_url_raw(add_query_arg('version', VENQORE_SYNC_VERSION, trailingslashit($site_url) . 'api/woo/plugin/check-update'));

    $response = wp_remote_get($update_url, ['timeout' => 5]);
    if (is_wp_error($response) || wp_remote_retrieve_response_code($response) !== 200) {
        return $result;
    }
    ...
    if (!empty($body)) {
        ...
        $obj->name          = $body['name'];
        $obj->slug          = $body['slug'];
        $obj->plugin_name   = $body['plugin'];
        $obj->version       = $body['new_version'];
        $obj->author        = '<a href="https://venqore.com">VenQore</a>';
        $obj->homepage      = $body['url'];
        $obj->download_link = $body['package'];
        $obj->sections      = $body['sections'];
        return $obj;
    }
