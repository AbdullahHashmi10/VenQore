    ...
    public function up(): void
    {
        ...
        if (in_array($driver, ['mysql', 'mariadb'], true)) {
            ...
            if (Schema::hasTable('journal_entries') && Schema::hasColumn('journal_entries', 'user_id')) {
                try { DB::statement('ALTER TABLE `journal_entries` DROP FOREIGN KEY `journal_entries_user_id_foreign`'); } catch (\Throwable $e) {}
                try { DB::statement('ALTER TABLE `journal_entries` DROP FOREIGN KEY `journal_entries_user_id_foreign_restrict`'); } catch (\Throwable $e) {}
                try {
                    DB::statement(
            ...
            if (Schema::hasTable('transactions') && Schema::hasColumn('transactions', 'party_id')) {
                try { DB::statement('ALTER TABLE `transactions` DROP FOREIGN KEY `transactions_party_id_foreign`'); } catch (\Throwable $e) {}
                try { DB::statement('ALTER TABLE `transactions` DROP FOREIGN KEY `transactions_party_id_foreign_restrict`'); } catch (\Throwable $e) {}
                try {
                    DB::statement(
