...
with open(os.path.join(artifact_dir, "feature_list.md"), "w", encoding="utf-8") as f:
    ...
    # Process the categorized features
    for label, items_block in categories:
        items = re.findall(r"\{\s*n:\s*'((?:[^'\\]|\\.)*)',\s*d:\s*'((?:[^'\\]|\\.)*)'\s*\}", items_block)
        for name, desc in items:
            name = name.replace("\\'", "'")
            desc = desc.replace("\\'", "'")
            f.write(f"{counter}. **{name}**: {desc}\n")
            counter += 1
            
    # Add backend & infrastructure features
    extra_features = [
        ("API Access & Webhooks", "Programmatic access and event-driven data streaming for third-party integrations."),
        ("Custom Domain Mapping", "White-label the application under your own URL."),
        ("Dedicated Account Manager", "Priority assigned support representative."),
        ("SSO / SAML Authentication", "Enterprise-grade single sign-on capabilities."),
        ("White-Glove Onboarding", "Full guided setup and data migration service."),
        ("Priority Email & Phone Support", "SLA-backed response times for critical operations.")
    ]
    
    for name, desc in extra_features:
        f.write(f"{counter}. **{name}**: {desc}\n")
        counter += 1

print(f"Extracted {counter-1} features in flat numbered list.")
