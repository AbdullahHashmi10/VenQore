# VenQore — Category 5 IDE Instruction
# C5.1 — `Heart/OneCoreReconciliationGate`
...
- COMMIT immediately after this test goes green (`git add -A && git commit -m "C5.1: OneCoreReconciliationGate — capstone proof gate green"`).
- DO NOT rename, remove, or soften any existing test in `Tester/tests/Feature/Core/` or `/Money/`.
...
Create ONE permanent test file:
`Tester/tests/Feature/Heart/OneCoreReconciliationGate.php`

This test seeds a complete business scenario covering **every transaction type** the system supports, then proves that:
1. **Journal integrity** — trial balance is zero (Σ debits = Σ credits) at every stage.
2. **Derived balance reconciliation** — each `Account`'s derived balance (from journal) matches the direct DB aggregate.
3. **Report reconciliation** — `FinancialReportingService` (FRS) numbers match direct DB aggregates to the cent.
4. **Dashboard card reconciliation** — the Inertia props returned by `DashboardController` match FRS for the same window.
5. **Receipt reconciliation** — the `party-statement` Inertia page `closingBalance` for the customer matches the AR account balance in the journal.
6. **Inventory reconciliation** — `getInventoryValue()` matches `SUM(remaining_qty * unit_cost)` across batches directly.

When this test is green, the brand claim "our maths is 100% correct" is **provably, permanently enforced**.

---

## STEP 1 — Create the directory

Create directory: `Tester/tests/Feature/Heart/`
...
Create file: `Tester/tests/Feature/Heart/OneCoreReconciliationGate.php`

Use the exact content below. Do NOT modify the assertion values — they are derived by hand from first principles. If any assertion fails, the ENGINE is wrong, not the test.
...
```
E:\Software\Xampp\php\php.exe artisan test Tester/tests/Feature/Heart/OneCoreReconciliationGate.php --verbose
```

**Expected result:** All tests pass. The output must show each test name with a green checkmark. No skipped, no failed.

If any test fails:
- READ the failure message completely.
- Identify which account code / which report method returned the wrong number.
- Fix the implementation (FRS method or the underlying query), NOT the assertion.
- Re-run the gate. Repeat until green.

---

## STEP 4 — Run the full guard suite to confirm no regression

```
E:\Software\Xampp\php\php.exe artisan test Tester/tests/Feature/Core Tester/tests/Feature/Money Tester/tests/Feature/Heart --compact
```

**Expected:** All green. The five permanent guard tests must pass:
`SingleWriterGuardTest`, `BalanceConsistencyTest`, `CalculatorParityTest`, `FifoDeterminismTest`, `DashboardConsistencyTest`, `NoSecondCalculatorTest`.

---

## STEP 5 — Commit

Once green (and ONLY once green):

```
git add -A
git commit -m "C5.1: OneCoreReconciliationGate — capstone proof gate green"
```
...
The instruction is COMPLETE when:
- [ ] `Tester/tests/Feature/Heart/OneCoreReconciliationGate.php` exists with all 9 test methods.
- [ ] All 9 tests pass (green) on MySQL `amd_pos_test`.
- [ ] The Core + Money + Heart suites run fully green with no regressions.
- [ ] Committed with message `C5.1: OneCoreReconciliationGate — capstone proof gate green`.

**Report back:** the RAW test output from Step 3 and Step 4. Do NOT mark done until the output is verified.
...
| Test | What it proves |
|---|---|
| `test_journal_is_balanced_after_cash_sale` | Every cash-sale journal entry balances (Σ D = Σ C) |
| `test_journal_is_balanced_after_credit_sale_and_partial_return` | Credit sale + partial return both balance; returns don't break double-entry |
| `test_journal_is_balanced_after_purchase_on_credit` | Purchase on credit produces a balanced AP/Inventory entry |
| `test_all_account_derived_balances_match_journal_direct_aggregate` | No account uses a stale `balance` column — ALL balances are journal-derived |
| `test_frs_report_numbers_match_direct_db_on_golden_transaction` | The single calculator (FRS) is mathematically correct on the canonical scenario |
| `test_per_cashier_revenue_sums_to_pl_total` | Per-user breakdown sums to the same P&L total (one source) |
| `test_item_wise_gp_reconciles_to_pl_gp` | Item-wise and P&L gross profit agree to the cent after returns |
| `test_dashboard_controller_revenue_matches_frs` | The live dashboard card reads from FRS, not a separate path |
| `test_party_statement_closing_balance_matches_ar_journal` | The customer receipt/ledger page shows the journal AR balance, not a cached value |
| `test_frs_inventory_value_matches_direct_batch_aggregate` | FRS inventory == batch table reality, no phantom inventory |
| `test_trial_balance_report_balances_to_zero` | The FRS trial balance report itself sums to zero |
| `test_balance_sheet_assets_equal_liabilities_plus_equity` | The accounting equation holds on the balance sheet report |
| `test_aged_receivables_total_matches_ar_account` | Aged AR total == journal AR (no double-counting, no missing invoices) |

---

*End of C5.1 instruction. When this is green and committed, the mathematical heart of VenQore is provably complete.*
