...
function Toggle({ value, onChange }) {
    return (
        <button type="button" onClick={() => onChange(!value)}
            ...
            <div className={`w-5 h-5 bg-white rounded-full shadow transition-all duration-normal ${value ? 'translate-x-5' : 'translate-x-0'}`} />
        </button>
    );
}
...
function FieldRow({ label, icon: Icon, children }) {
    return (
        <div className="flex items-start gap-4 py-3.5 border-b border-neutral-800/60 last:border-0">
            <div className="flex items-center gap-2 w-48 shrink-0 pt-0.5">
                {Icon && <Icon size={14} className="text-ink-muted shrink-0" />}
                <span className="text-xs font-bold text-ink-muted uppercase tracking-widest">{label}</span>
            </div>
...
function LimitCard({ limitKey, info, tenant, availableKeys }) {
    ...
    return (
        <div className={`rounded-2xl border p-4 transition-all ${hasOverride ? 'bg-amber-500/5 border-amber-500/30' : 'bg-neutral-900 border-neutral-800 hover:border-line-strong'}`}>
            <div className="flex items-center justify-between mb-2">
                <span className="font-mono text-xs text-ink-muted uppercase tracking-wider">{limitKey}</span>
                <div className="flex items-center gap-1.5">
                    {hasOverride ? (
                        <>
                            <span className="flex items-center gap-1 text-2xs font-bold text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-full uppercase">
                                <Zap size={9} /> Override
                            </span>
                        ...
                        <span className="flex items-center gap-1 text-2xs font-bold text-ink-secondary bg-neutral-800 px-2 py-0.5 rounded-full uppercase">
            ...
            {!editing ? (
                <div className="flex items-end justify-between">
                    <div>
                        ...
                        {hasOverride && (
                            <div className="text-xs text-ink-secondary mt-0.5">
                                Plan default: <span className="text-ink-muted">{info.plan_default === null ? '∞' : String(info.plan_default)}</span>
                            </div>
                        )}
                    ...
                    <button onClick={() => setEditing(true)}
                        className="p-2 text-ink-secondary hover:text-brand-400 hover:bg-interactive-hover rounded-xl transition-all" title="Edit">
                        <Edit2 size={13} />
                    </button>
                </div>
            ) : (
    ...
    return (
        <OneGlanceLayout title={`Store — ${tenant.name}`} mode="admin" activeMenu="Tenant Overrides">
            ...
            <div className="flex items-center gap-3 mb-6">
                ...
                <span className="text-neutral-300 font-bold">{tenant.name}</span>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="xl:col-span-2 space-y-6">
                    ...
                    <div className="bg-neutral-900 rounded-2xl border border-neutral-800 overflow-hidden">
                        <div className="flex items-center gap-3 px-6 py-4 border-b border-neutral-800">
                            ...
                            <h3 className="font-bold text-white text-sm uppercase tracking-widest">Store Identity</h3>
                        </div>
                        <form onSubmit={saveProfile} className="px-6 py-2">
                            ...
                            <FieldRow label="Owner" icon={User}>
                                <div className="flex flex-col gap-0.5">
                                    ...
                                    <span className="text-xs text-ink-muted">{tenant.owner_email}</span>
                                </div>
                            </FieldRow>
                            <FieldRow label="Store ID" icon={Hash}>
                                <span className="text-sm text-ink-muted font-mono">#{tenant.id}</span>
                            </FieldRow>
                            <FieldRow label="Slug" icon={Tag}>
                                <span className="text-sm text-ink-muted font-mono">{tenant.slug}</span>
                            </FieldRow>
                            ...
                            {/* Billing */}
                            <div className="flex items-center gap-3 mt-4 mb-2 pt-4 border-t border-neutral-800">
                                <DollarSign size={14} className="text-emerald-400" />
                                <span className="text-xs font-bold text-ink-muted uppercase tracking-widest">Billing & Plan</span>
                            </div>
                            ...
                            <div className="flex items-center gap-3 mt-4 mb-2 pt-4 border-t border-neutral-800">
                                ...
                                <span className="text-xs font-bold text-ink-muted uppercase tracking-widest">Locale & Currency</span>
                            </div>
                            ...
                            <div className="flex items-center gap-3 mt-4 mb-2 pt-4 border-t border-neutral-800">
                                ...
                                <span className="text-xs font-bold text-ink-muted uppercase tracking-widest">Feature Flags</span>
                            </div>
                            {[
                                ...
                                <FieldRow key={key} label={label}>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className={`text-xs font-bold ${data[key] ? 'text-emerald-400' : 'text-ink-secondary'}`}>
                            ...
                            <div className="pt-5 pb-4 border-t border-neutral-800 mt-4">
                                <button type="submit" disabled={processing}
                    ...
                    <div className="bg-neutral-900 rounded-2xl border border-neutral-800 overflow-hidden">
                        <div className="flex items-center gap-3 px-6 py-4 border-b border-neutral-800">
                            ...
                            <h3 className="font-bold text-white text-sm uppercase tracking-widest">Override History</h3>
                        </div>
                        {override_history.length === 0 ? (
                            <div className="p-12 text-center">
                                <Info size={32} className="mx-auto mb-3 text-ink-secondary" />
                                <p className="text-ink-secondary font-medium">No overrides applied yet.</p>
                            </div>
                        ) : (
                                    ...
                                    <thead>
                                        <tr className="bg-neutral-800/50">
                                            {['Key', 'Value', 'Original', 'Reason', 'Expires', ''].map(h => (
                                                <th key={h} className="px-4 py-3 text-left text-xs font-bold text-ink-muted uppercase tracking-widest">{h}</th>
                                            ))}
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {override_history.map((o, i) => {
                                            ...
                                            return (
                                                <tr key={o.id} className={`border-t border-neutral-800 ${isExpired ? 'opacity-40' : 'hover:bg-interactive-hover'}`}>
                                                    <td className="px-4 py-3 font-mono text-xs text-brand-400">{o.override_key}</td>
                                                    <td className="px-4 py-3 font-bold text-white">{o.override_value ?? <span className="text-ink-muted">∞</span>}</td>
                                                    <td className="px-4 py-3 text-ink-muted">{o.original_value ?? '—'}</td>
                                                    <td className="px-4 py-3 text-ink-muted max-w-[160px] truncate">{o.reason || '—'}</td>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-neutral-900 rounded-2xl border border-neutral-800 p-5">
                        <h3 className="text-xs font-bold text-ink-muted uppercase tracking-widest mb-4">Live Usage</h3>
                        <div className="grid grid-cols-3 gap-3">
                            {[
                                ...
                                <div key={label} className="bg-neutral-800/60 rounded-2xl p-3 text-center">
                                    <Icon size={16} className={`${color} mx-auto mb-1`} />
                                    <div className="text-xl font-bold text-white">{value}</div>
                                    <div className="text-2xs font-bold text-ink-muted uppercase">{label}</div>
                                </div>
                            ))}
                        </div>
                        <div className="mt-4 pt-4 border-t border-neutral-800 flex flex-wrap gap-2">
                            <span className={`px-3 py-1 rounded-full text-xs font-bold border uppercase ${statusColor[tenant.status] ?? 'bg-neutral-800 text-ink-muted border-neutral-700'}`}>
                                {tenant.status}
                            </span>
                            <span className="px-3 py-1 rounded-full text-xs font-bold border bg-brand-500/10 text-brand-400 border-brand-500/20 uppercase">
                                {tenant.plan}
                            </span>
                            <span className="px-3 py-1 rounded-full text-xs font-bold border bg-neutral-800 text-ink-muted border-neutral-700">
                    ...
                    <div className="bg-neutral-900 rounded-2xl border border-neutral-800 overflow-hidden">
                        <div className="flex items-center gap-3 px-5 py-4 border-b border-neutral-800">
                            ...
                            <h3 className="font-bold text-white text-sm uppercase tracking-widest">Plan Limits</h3>
                            <span className="ml-auto text-xs text-ink-secondary font-semibold">{Object.keys(effective_limits).length} keys</span>
                        </div>
