...
import {
    Activity, DollarSign, TrendingUp, Cpu, Users, ArrowUpRight,
    Shield, Clock, Search, AlertTriangle, ExternalLink, Calendar,
    Zap, Layers, BarChart3, Database, Key
} from 'lucide-react';
...
export default function PlatformAiUsageIndex({
    ...
    return (
        <OneGlanceLayout title="AI Billing & Usage" mode="admin" activeMenu="Tenant Overrides">
            <Head title="Platform AI Cost & Consumption — VenQore" />
            ...
            <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                        <div className="flex items-center gap-2 text-xs font-semibold text-neutral-400 mb-2">
                            ...
                            <span>/</span>
                            <span className="text-[#0BAA8F]">AI Operations</span>
                        </div>
                        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-white flex items-center gap-3">
                            <Cpu className="text-[#0BAA8F]" size={28} />
                            <span>AI Billing & Consumption</span>
                        </h1>
                        <p className="text-xs text-neutral-400 mt-1 max-w-xl leading-relaxed">
                            Monitor infrastructure spend across foundation models, reconcile provider invoices, and audit high-volume tenant usage.
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                    ...
                    {/* Today's Spend */}
                    <div className="bg-[#0D1322]/90 rounded-2xl border border-white/[0.08] p-5 shadow-xl backdrop-blur-sm space-y-2">
                    ...
                    <div className="bg-[#0D1322]/90 rounded-2xl border border-white/[0.08] p-5 shadow-xl backdrop-blur-sm space-y-2">
                        <div className="flex items-center justify-between text-neutral-400">
                            <span className="text-xs font-bold uppercase tracking-wider">Month-to-Date Spend</span>
                        ...
                        <div className="text-xs font-mono text-neutral-400 flex items-center justify-between pt-1">
                            ...
                            <span className="text-neutral-500">{new Date().toLocaleDateString('default', { month: 'short' })}</span>
                        </div>
                    </div>
                    ...
                    <div className="bg-[#0D1322]/90 rounded-2xl border border-white/[0.08] p-5 shadow-xl backdrop-blur-sm space-y-2">
                        ...
                        <div className="text-3xl font-bold font-mono tracking-tight text-purple-300">
                            ${(kpis.projected_month_end || 0).toFixed(2)}
                        </div>
                        <div className="text-xs font-mono text-neutral-400 flex items-center justify-between pt-1">
                            <span>~${(kpis.avg_daily_spend_7d || 0).toFixed(2)} / day avg</span>
                            <span className="text-purple-400 font-bold">Month-end</span>
                        </div>
                    </div>
                ...
                <div className="bg-[#0D1322]/90 rounded-2xl border border-white/[0.08] p-6 shadow-xl backdrop-blur-sm space-y-5">
                    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.08] pb-4">
                        <div className="flex items-center gap-2.5">
                            <BarChart3 size={18} className="text-[#0BAA8F]" />
                            <div>
                                <h3 className="font-bold text-white text-base">30-Day Cost & Call Trajectory</h3>
                                <p className="text-xs text-neutral-400">Daily platform provider expenditure (excluding BYOK)</p>
                            </div>
                        </div>
                    ...
                    {(!daily_trend || daily_trend.length === 0) ? (
                        <div className="text-center py-10 text-neutral-500 text-xs italic">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="h-44 flex items-end gap-1.5 pt-4 pb-2">
                                {daily_trend.map((d) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            {/* Tooltip */}
                                            <div className="absolute bottom-full mb-2 hidden group-hover:flex flex-col items-center bg-neutral-900 border border-white/20 p-2 rounded-xl text-[10px] font-mono z-20 whitespace-nowrap shadow-xl">
                                                <span className="text-neutral-400 font-bold">{d.date}</span>
                                                <span className="text-white font-bold">${cost.toFixed(4)}</span>
                                                <span className="text-neutral-500">{d.total_calls} calls</span>
                                            </div>
                            ...
                            {/* X-axis labels */}
                            <div className="flex items-center justify-between text-[10px] font-mono text-neutral-500 pt-1 border-t border-white/[0.05]">
                ...
                <div className="bg-[#0D1322]/90 rounded-2xl border border-white/[0.08] p-6 shadow-xl backdrop-blur-sm space-y-5">
                    <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/[0.08] pb-4">
                        <div className="flex items-center gap-2.5">
                            ...
                            <div>
                                <h3 className="font-bold text-white text-base">Per-Tenant AI Consumption</h3>
                                <p className="text-xs text-neutral-400">Stores ranked by 30-day platform spend</p>
                            </div>
                        </div>
                                    ...
                                    {filteredTenants.map((t) => (
                                        <tr key={t.tenant_id} className="hover:bg-white/[0.02] transition-colors">
                                            <td className="py-3 px-3">
                                                ...
                                                <div className="font-mono text-[10px] text-neutral-500">#{t.tenant_id} · {t.slug}</div>
                                            </td>
                                            <td className="py-3 px-3">
                                                <div className="flex items-center gap-1.5">
                                                    <span className="px-2 py-0.5 rounded-full text-[10px] font-mono font-bold uppercase bg-white/5 border border-white/10 text-neutral-300">
                                                        {t.plan}
                                                    </span>
                                                    {t.ai_status === 'byok' ? (
                                            ...
                                            <td className="py-3 px-3 font-mono text-neutral-300 capitalize">
                                                {t.top_feature || 'scan'}
                                            </td>
                                            ...
                                            <td className="py-3 px-3 font-mono font-bold">
                                                {t.ai_status === 'byok' ? (
                                                    <span className="text-emerald-400">$0.00 (Self-funded)</span>
                                                ) : (
                                                    <span className="text-white">${(t.cost_usd || 0).toFixed(4)}</span>
                                                )}
                                            </td>
                ...
                <div className="bg-[#0D1322]/90 rounded-2xl border border-white/[0.08] p-6 shadow-xl backdrop-blur-sm space-y-5">
                    <div className="flex items-center justify-between border-b border-white/[0.08] pb-4">
                        <div className="flex items-center gap-2.5">
                            ...
                            <div>
                                <h3 className="font-bold text-white text-base">Model & Provider Reconciliation</h3>
                                <p className="text-xs text-neutral-400">Match against Google Cloud & OpenAI invoice line items</p>
                            </div>
                        </div>
                            ...
                            <table className="w-full text-xs">
                                ...
                                <tbody className="divide-y divide-white/[0.04]">
                                    {model_breakdown.map((m) => (
                                        <tr key={`${m.provider}-${m.model}`} className="hover:bg-white/[0.02] transition-colors">
