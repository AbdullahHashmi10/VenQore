        ...
        <OneGlanceLayout mode="admin" activeMenu="Digital Products" title="Digital Products & Registry Hub">
            ...
            <div className="space-y-6">
                ...
                <div className="p-8 rounded-3xl bg-slate-900 border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div>
                        <h2 className="text-2xl font-black text-white mb-2">VenQore Master Registry Control</h2>
                ...
                {/* Tab content area */}
                <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden min-h-[500px]">
                    {activeTab === 'chats' && (
                        <div className="grid grid-cols-1 lg:grid-cols-12 h-[600px] divide-y lg:divide-y-0 lg:divide-x divide-slate-800">
                            {/* Left Side: Ticket List */}
                            <div className="lg:col-span-4 flex flex-col h-full overflow-hidden">
                                <div className="p-4 bg-slate-950/20 border-b border-slate-800 flex items-center gap-3">
                                ...
                                <div className="flex-1 overflow-y-auto divide-y divide-slate-800/40 custom-scrollbar">
                                    {loadingChats ? (
                                        <div className="p-8 text-center text-slate-500 text-xs">Loading active channels...</div>
                                    ) : filteredChats.length === 0 ? (
                                        <div className="p-8 text-center text-slate-500 text-xs">No active chat requests found.</div>
                                    ) : (
                                        filteredChats.map(chat => (
                                            <div
                                                key={chat.id}
                                                onClick={() => setSelectedChat(chat)}
                                                className={`p-4 cursor-pointer hover:bg-slate-800/40 transition-colors flex flex-col gap-2 ${
                                                    selectedChat?.id === chat.id ? 'bg-slate-800/60' : ''
                                                }`}
                                            >
                                ...
                                {selectedChat ? (
                                    <>
                                        ...
                                        <div className="p-6 border-b border-slate-800 bg-slate-900/50 flex items-center justify-between">
                                            <div>
                                                <h3 className="font-black text-white text-base">{selectedChat.requester_name}</h3>
                                                <p className="text-slate-500 text-xs">{selectedChat.requester_email}</p>
                                            </div>
                                        ...
                                        {/* Input Box */}
                                        <form onSubmit={handleSendReply} className="p-4 border-t border-slate-800 bg-slate-900 flex items-center gap-3">
                                            <input
                                                type="text"
                                                value={replyBody}
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 divide-y lg:divide-y-0 lg:divide-x divide-slate-800 min-h-[500px]">
                            ...
                            <div className="lg:col-span-5 p-6 space-y-6">
                                ...
                                <form onSubmit={handleCreateProduct} className="space-y-4 text-xs">
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="text-slate-400 font-bold block">Product Description</label>
                                        <textarea
                                            rows={3}
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-1.5">
                                            <label className="text-slate-400 font-bold block">Version / Tag</label>
                                            <input
                                                type="text"
                                        ...
                                        <div className="space-y-1.5">
                                            <label className="text-slate-400 font-bold block">Development Status</label>
                                            <select
                                    ...
                                    <div className="space-y-3 pt-2">
                                        <div className="flex items-center justify-between">
                                            <label className="text-slate-400 font-bold">Platform Purchase Links</label>
                                            <button
                                                type="button"
                            ...
                            <div className="lg:col-span-7 p-6 flex flex-col h-full overflow-hidden">
                                ...
                                <div className="space-y-3 overflow-y-auto max-h-[420px] custom-scrollbar pr-2">
                                    {loadingProducts ? (
                                        <div className="p-8 text-center text-slate-500 text-xs">Loading products catalog...</div>
                                    ) : products.length === 0 ? (
                                        <div className="p-8 text-center text-slate-500 text-xs">No products cataloged. Add your first listing using the left panel.</div>
                                    ) : (
                                        products.map(prod => (
                                            <div 
                                                key={prod.id}
                                            ...
                                            >
                                                <div className="space-y-1 text-xs flex-1 cursor-pointer" onClick={() => handleStartEdit(prod)}>
                                                    <div className="flex items-center gap-2">
                                                        <h4 className="font-bold text-white text-sm hover:text-indigo-400 transition-colors flex items-center gap-1.5">
                                                            {prod.name}
