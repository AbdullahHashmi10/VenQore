    ...
    return (
        ...
        >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
                ...
                {trend != null && (
                    <span style={{ marginLeft: 'auto', fontSize: 12, fontWeight: 700, color: trend >= 0 ? 'rgb(var(--vq-emerald-500))' : 'rgb(var(--vq-red-500))', display: 'flex', alignItems: 'center', gap: 3 }}>
                        {trend >= 0 ? <TrendingUp size={12} /> : <TrendingDown size={12} />}
                    </span>
                )}
            </div>
            <div style={{ fontSize: 22, fontWeight: 800, color: 'var(--text-main, rgb(var(--vq-slate-900)))', lineHeight: 1 }}>{value}</div>
            <div style={{ fontSize: 12, color: 'var(--text-sub, rgb(var(--vq-slate-500)))', marginTop: 5, fontWeight: 600 }}>{label}</div>
            {sub && <div style={{ fontSize: 11, color: 'rgb(var(--vq-slate-400))', marginTop: 3 }}>{sub}</div>}
        </div>
    );
}
...
function AgingRow({ bucket, amount, pct, color }) {
    return (
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
            <div style={{ width: 80, fontSize: 11, fontWeight: 700, color: 'rgb(var(--vq-slate-500))' }}>{bucket}</div>
            <div style={{ flex: 1, height: 6, background: vq.blue[50], borderRadius: 4, overflow: 'hidden' }}>
                <div style={{ width: `${pct}%`, height: '100%', background: color, borderRadius: 4, transition: 'width 0.6s ease' }} />
            </div>
            <div style={{ width: 90, textAlign: 'right', fontSize: 13, fontWeight: 700, color: 'var(--text-main, rgb(var(--vq-slate-900)))' }}>{amount}</div>
        </div>
    );
}
        ...
        <OneGlanceLayout activeMenu="Dashboard">
            ...
            <div style={{ padding: '24px 24px 48px', maxWidth: 1400, margin: '0 auto' }}>
                ...
                <div style={{ marginBottom: 28 }}>
                    <h1 style={{ fontSize: 22, fontWeight: 800, color: 'var(--text-main, rgb(var(--vq-slate-900)))', margin: 0 }}>
                        💰 Finance Overview
                    </h1>
                    <p style={{ fontSize: 13, color: 'rgb(var(--vq-slate-500))', margin: '4px 0 0' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 20, marginBottom: 24 }}>
                    {/* P&L Chart */}
                    <div style={{ background: 'var(--card-bg,#fff)', border: '1px solid var(--card-border,rgb(var(--vq-slate-100)))', borderRadius: 20, padding: 24, boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
                        <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--text-main,rgb(var(--vq-slate-900)))', marginBottom: 20 }}>
                        ...
                        <ResponsiveContainer width="100%" height={200}>
                            <BarChart data={plChartData ?? []} barGap={4}>
                                ...
                                <Tooltip formatter={(v) => fmt(v)} contentStyle={{ borderRadius: 10, border: '1px solid rgb(var(--vq-slate-200))', fontSize: 12 }} />
                    ...
                    {/* Receivables Aging */}
                    <div style={{ background: 'var(--card-bg,#fff)', border: '1px solid var(--card-border,rgb(var(--vq-slate-100)))', borderRadius: 20, padding: 24, boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
                        <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--text-main,rgb(var(--vq-slate-900)))', marginBottom: 8 }}>
                            Receivables Aging
                        </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                    ...
                    <div style={{ background: 'var(--card-bg,#fff)', border: '1px solid var(--card-border,rgb(var(--vq-slate-100)))', borderRadius: 20, padding: 24, boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                            <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--text-main,rgb(var(--vq-slate-900)))' }}>Recent Journal Entries</div>
                            {(pendingJournalCount ?? 0) > 0 && (
                                <span style={{ padding: '3px 10px', borderRadius: 8, background: 'rgb(var(--vq-amber-100))', color: 'rgb(var(--vq-amber-600))', fontSize: 11, fontWeight: 700 }}>
                                    {pendingJournalCount} pending
                                </span>
                            )}
                        </div>
                        {(recentJournalEntries ?? []).length === 0 ? (
                            <div style={{ padding: '24px 0', textAlign: 'center', color: 'rgb(var(--vq-slate-400))', fontSize: 13 }}>No recent entries.</div>
                        ) : (
                            (recentJournalEntries ?? []).slice(0, 6).map((entry, i) => (
                                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 0', borderBottom: '1px solid rgb(var(--vq-slate-50))' }}>
                                    <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'rgb(var(--vq-indigo-500))', flexShrink: 0 }} />
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-main,rgb(var(--vq-slate-900)))', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{entry.description ?? 'Journal Entry'}</div>
                                        <div style={{ fontSize: 11, color: 'rgb(var(--vq-slate-400))' }}>{entry.date}</div>
                                    </div>
                                    <div style={{ fontSize: 13, fontWeight: 700, color: entry.type === 'debit' ? 'rgb(var(--vq-emerald-500))' : 'rgb(var(--vq-amber-500))', flexShrink: 0 }}>{fmt(entry.amount)}</div>
                                </div>
                            ))
                        )}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        <div style={{ background: 'var(--card-bg,#fff)', border: '1px solid var(--card-border,rgb(var(--vq-slate-100)))', borderRadius: 20, padding: 24, boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
                            <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--text-main,rgb(var(--vq-slate-900)))', marginBottom: 14 }}>Bank &amp; Cash Accounts</div>
                            {[...(bankAccounts ?? []), ...(cashAccounts ?? [])].map((acct, i) => (
                                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid rgb(var(--vq-slate-50))', fontSize: 13 }}>
                                    <span style={{ color: 'rgb(var(--vq-slate-600))', fontWeight: 500 }}>{acct.name ?? acct.bank_name ?? 'Account'}</span>
                                    <span style={{ fontWeight: 700, color: (acct.current_balance ?? 0) >= 0 ? 'rgb(var(--vq-emerald-500))' : 'rgb(var(--vq-red-500))' }}>{fmt(acct.current_balance ?? 0)}</span>
                                </div>
                            ))}
                            {[...(bankAccounts ?? []), ...(cashAccounts ?? [])].length === 0 && (
                                <div style={{ color: 'rgb(var(--vq-slate-400))', fontSize: 13, padding: '12px 0' }}>No accounts configured.</div>
                            )}
                        </div>

                        {/* Quick Actions */}
                        <div style={{ background: 'var(--card-bg,#fff)', border: '1px solid var(--card-border,rgb(var(--vq-slate-100)))', borderRadius: 20, padding: 20, boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
                            <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--text-main,rgb(var(--vq-slate-900)))', marginBottom: 12 }}>Quick Actions</div>
