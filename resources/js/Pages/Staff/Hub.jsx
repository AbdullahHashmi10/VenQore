...
function RoleBadge({ role }) {
    ...
    const Icon = cfg.icon;
    return (
        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider border ${cfg.color} ${cfg.bg} ${cfg.border}`}>
        ...
        <div className="min-h-screen bg-[#02000f] text-white font-sans selection:bg-violet-500/40 selection:text-white relative overflow-hidden">
            ...
            <header className="relative z-10 w-full max-w-7xl mx-auto px-6 py-6 flex items-center justify-between border-b border-white/[0.04]">
                ...
                >
                    ...
                    <div className="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center group-hover:bg-red-500/20 group-hover:border-red-500/40 transition-all">
                        <LogOut size={13} className="text-slate-400 group-hover:text-red-400" />
                    </div>
                </button>
            </header>
            ...
            <main className="relative z-10 w-full max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="relative rounded-[2rem] bg-gradient-to-br from-white/[0.03] to-white/[0.01] border border-white/[0.06] p-8 overflow-hidden shadow-2xl">
                        ...
                        <p className="text-slate-400 text-sm max-w-lg mb-6 leading-relaxed">
                            Authorized as <span className="text-slate-300 font-semibold">{employee.email}</span>. Operating at the platform control level.
                        </p>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="group rounded-2xl bg-white/[0.02] border border-white/[0.05] p-5 transition-all duration-300 flex flex-col justify-between shadow-lg">
                                <div>
                                    ...
                                    <h3 className="font-bold text-white mb-1 group-hover:text-indigo-300 transition-colors">Support Room</h3>
                                    <p className="text-xs text-slate-500 mb-4">Manage global customer inbox sessions and co-pilot with Vena AI.</p>
                                ...
                                {hasSupportAccess ? (
                                    <Link href={route('platform.chatbot.inbox')} className="w-full text-center py-2.5 rounded-xl bg-violet-600 hover:bg-violet-500 text-xs font-bold text-white shadow-lg shadow-violet-600/15 hover:shadow-violet-600/30 transition-all flex items-center justify-center gap-1.5">
                                        Enter Support Command <ArrowRight size={12} />
                                    </Link>
                                ) : (
                                    <div className="w-full text-center py-2.5 rounded-xl bg-slate-800 text-slate-500 text-xs font-bold flex items-center justify-center gap-1.5 cursor-not-allowed">
                            ...
                            <div className="group rounded-2xl bg-white/[0.01] border border-white/[0.03] p-5 opacity-60 flex flex-col justify-between shadow-lg">
                                <div>
                                    ...
                                    <h3 className="font-bold text-slate-300 mb-1">Content & SEO Room</h3>
                                    <p className="text-xs text-slate-600 mb-6">VenQore landing page articles, knowledge bases, and site SEO tools.</p>
                                </div>
                                <div className="w-full text-center py-2.5 rounded-xl bg-slate-900 text-slate-600 text-xs font-bold flex items-center justify-center gap-1.5">
                            ...
                            <div className="group rounded-2xl bg-white/[0.01] border border-white/[0.03] p-5 opacity-60 flex flex-col justify-between shadow-lg">
                                <div>
                                    ...
                                    <h3 className="font-bold text-slate-300 mb-1">Marketing & Growth</h3>
                                    <p className="text-xs text-slate-600 mb-6">Social reach analytics, marketing lists, and promotional assets.</p>
                                </div>
                                <div className="w-full text-center py-2.5 rounded-xl bg-slate-900 text-slate-600 text-xs font-bold flex items-center justify-center gap-1.5">
                                    <Lock size={12} /> Coming Soon
                                </div>
                            ...
                            <div className="group rounded-2xl bg-white/[0.01] border border-white/[0.03] p-5 opacity-60 flex flex-col justify-between shadow-lg">
                                <div>
                                    ...
                                    <h3 className="font-bold text-slate-300 mb-1">Finance & Subscriptions</h3>
                                    <p className="text-xs text-slate-600 mb-6">Active subscription fee journals and Gateway reconciliations.</p>
                                </div>
                                <div className="w-full text-center py-2.5 rounded-xl bg-slate-900 text-slate-600 text-xs font-bold flex items-center justify-center gap-1.5">
                                    <Lock size={12} /> Coming Soon
                                </div>
                        ...
                        <div className="space-y-4">
                            ...
                            {referred_chats.length === 0 ? (
                                <div className="rounded-2xl border border-dashed border-white/[0.06] bg-white/[0.01] p-12 text-center">
                                    ...
                                    <p className="text-slate-500 text-xs mt-1">There are currently no chatbot session tickets referred specifically to your username.</p>
                                </div>
                            ) : (
                ...
                <div className="space-y-8">
                    ...
                    {/* Task Checklist card */}
                    <div className="rounded-[2rem] bg-gradient-to-b from-white/[0.03] to-white/[0.01] border border-white/[0.06] p-6 shadow-2xl">
                        ...
                        <div className="space-y-3.5">
                            {activeTasks.map(task => (
                                <button
                                    ...
                                    className={`w-full text-left p-4 rounded-xl border flex items-start gap-3 transition-all duration-300 group ${
                                        task.completed 
                                            ? 'bg-emerald-500/[0.02] border-emerald-500/15 text-slate-500 line-through' 
                                            : 'bg-white/[0.01] border-white/[0.05] hover:border-white/[0.1] text-slate-300'
                                    }`}
                                >
                                    <div className="shrink-0 mt-0.5">
                                        {task.completed ? (
                                            ...
                                            <div className="w-4 h-4 rounded border border-slate-700 flex items-center justify-center group-hover:border-violet-500 transition-colors" />
                                        )}
                                    </div>
                                    <div className="min-w-0">
                    ...
                    <div className="rounded-[2rem] bg-white/[0.02] border border-white/[0.05] p-6 relative overflow-hidden">
            ...
            {employee.role === 'owner' && (
                <section className="relative z-10 w-full max-w-7xl mx-auto px-6 pb-24 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="border-t border-white/[0.04] pt-12">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                            <div>
                                <h2 className="text-2xl font-black text-white tracking-tight flex items-center gap-2.5" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div className="lg:col-span-2 rounded-[2rem] bg-gradient-to-br from-white/[0.03] to-white/[0.01] border border-white/[0.06] p-6 shadow-2xl flex flex-col justify-between h-[360px]">
                                ...
                                <div className="flex-1 min-h-0 w-full">
                                    {loadingStats ? (
                                        <div className="h-full w-full flex items-center justify-center text-slate-500 text-xs gap-2">
                            ...
                            <div className="rounded-[2rem] bg-gradient-to-b from-white/[0.03] to-white/[0.01] border border-white/[0.06] p-6 shadow-2xl flex flex-col justify-between">
                                ...
                                <div className="flex-1 space-y-4 overflow-y-auto custom-scrollbar pr-1">
                                    {loadingStats ? (
                                        <div className="h-48 w-full flex items-center justify-center text-slate-500 text-xs gap-2">
