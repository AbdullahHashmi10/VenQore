...
const RelatedPost = ({ post, index }) => (
    <RevealOnScroll delay={index * 0.1}>
        <Link href={`/blog/${post.slug}`} className="block group">
            <div className="rounded-[2rem] bg-white/[0.02] border border-white/[0.06] p-6 hover:border-teal-500/20 hover:bg-white/[0.04] hover:-translate-y-1 transition-all duration-500">
                ...
                <h4 className="text-base font-bold text-white tracking-tight leading-snug mb-2 group-hover:text-teal-100 transition-colors line-clamp-2">
                    {post.title}
                </h4>
        ...
        <div className="space-y-6">
            {paragraphs.map((para, i) => {
                ...
                if (trimmed.startsWith('> ')) {
                    return (
                        <blockquote key={i} className="border-l-2 border-teal-500/40 pl-6 py-2 text-lg text-teal-200/80 italic leading-relaxed font-medium">
                            {trimmed.replace(/^>\s*/, '')}
                        </blockquote>
                    );
                }
                        ...
                        <ul key={i} className="space-y-3 pl-1">
                            {items.map((item, j) => (
                                <li key={j} className="flex items-start gap-3 text-slate-400 leading-relaxed">
                                    <div className="w-1.5 h-1.5 rounded-full bg-teal-500/50 mt-2.5 flex-shrink-0" />
        ...
        >
            ...
            <section className="relative pt-40 pb-16 px-6">
                <div className="max-w-3xl mx-auto">
                    {/* Back link */}
                    <RevealOnScroll>
                        <Link href="/blog" className="inline-flex items-center gap-2 text-sm text-slate-600 hover:text-teal-400 transition-colors mb-12 group">
                    ...
                    <RevealOnScroll delay={0.05}>
                        <div className="flex items-center gap-4 mb-8">
                            <span className="px-3 py-1.5 rounded-full bg-teal-500/10 border border-teal-500/20 text-teal-400 text-[9px] font-black tracking-[0.2em] uppercase">
                                {post.category}
                            </span>
                    ...
                    <RevealOnScroll delay={0.2}>
                        <div className="flex items-center justify-between py-6 border-y border-white/5">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-full bg-teal-500/10 flex items-center justify-center text-teal-400 text-xs font-black">
                    ...
                    <div className="max-w-5xl mx-auto">
                        <RevealOnScroll>
                            <div className="flex items-center justify-between mb-12">
                                ...
                                <Link href="/blog" className="text-teal-400 text-xs font-bold uppercase tracking-[0.15em] flex items-center gap-2 hover:gap-3 transition-all">
                ...
                <RevealOnScroll>
                    <div className="max-w-2xl mx-auto">
                        <h2 className="text-3xl md:text-4xl font-black text-white tracking-tighter font-display mb-4">
                            Ready The See the <span className="text-teal-400">Difference?</span>
                        </h2>
