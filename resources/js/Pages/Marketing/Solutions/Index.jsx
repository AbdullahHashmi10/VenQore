...
export default function Index() {
    return (
        <MarketingLayout>
                    ...
                    {solutionsHubList.map((sol, i) => {
                        ...
                        return (
                            <RevealOnScroll key={sol.slug} delay={i * 0.1} direction="up">
                                <GlassCard className="h-full flex flex-col justify-between p-8 rounded-3xl border border-slate-200 dark:border-slate-800 hover:border-indigo-500/40 transition-all duration-300">
                                    <div>
                                        <div className="flex items-center justify-between mb-6">
                                            ...
                                            <span className={`text-2xs font-black uppercase px-3 py-1 rounded-full ${sol.badgeColor === 'emerald' ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : sol.badgeColor === 'indigo' ? 'bg-indigo-500/10 text-indigo-500 border border-indigo-500/20' : 'bg-slate-500/10 text-slate-500 border border-slate-500/20'}`}>
                                                {sol.badge}
                                            </span>
                                        </div>
                                    ...
                                    <div>
                                        <Link
                                            href={sol.href}
                                            className="inline-flex items-center text-sm font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 group"
                                        >
            ...
            {/* Proof Guarantee */}
            <section className="py-20 px-6 bg-slate-900 text-white rounded-3xl max-w-7xl mx-auto mb-24">
