...
import { ArrowRight, MessageCircle, Lock, Menu, X } from 'lucide-react';

/* ═══════════════════════════════════════════════════════════════════════════
   SHARED MARKETING LAYOUT — Midnight Nebula 2.0
   Upgraded background (gradient + beams + glows + grain, no images),
   scroll-progress, particle field and cursor spotlight — matches the landing.
...
export const MagneticButton = ({ children, href, className = '', variant = 'primary', ...props }) => {
    ...
    const baseClass = variant === 'primary'
        ? 'px-9 py-4 bg-white text-[#05030f] font-black text-[15px] rounded-full shadow-[0_8px_40px_-8px_rgba(255,255,255,0.35)] hover:shadow-[0_0_70px_-6px_rgba(165,180,252,0.55)]'
        : variant === 'ghost'
            ? 'px-8 py-4 bg-white/[0.04] border border-white/12 text-white font-bold text-[15px] rounded-full hover:bg-white/[0.08] hover:border-white/25 backdrop-blur-md'
...
export const SectionLabel = ({ children, icon: Icon }) => (
    <div className="inline-flex items-center gap-2.5 px-5 py-2 rounded-full bg-indigo-500/10 border border-indigo-400/20 text-indigo-300 text-[10px] font-black tracking-[0.3em] uppercase mb-8 backdrop-blur-sm">
        {Icon && <Icon size={13} />}
        {children}
    </div>
);
...
export const GlassCard = ({ children, className = '', hover = true, padding = 'p-8', ...props }) => (
    <div
        className={`relative ${padding} rounded-[2rem] bg-white/[0.03] border border-white/[0.07] backdrop-blur-sm ${hover ? 'hover:bg-white/[0.06] hover:border-indigo-500/20 hover:shadow-2xl hover:shadow-indigo-900/10 hover:-translate-y-1' : ''} transition-all duration-500 group ${className}`}
        {...props}
    >
        {children}
    </div>
...
const MkAmbient = () => (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
        ...
        <div className="absolute top-[-26%] left-[-16%] w-[52vw] h-[52vw] rounded-full blur-[190px] vqm-blob" style={{ background: 'radial-gradient(circle, rgba(99,102,241,0.15), transparent 62%)' }} />
        <div className="absolute top-[-22%] right-[-16%] w-[48vw] h-[48vw] rounded-full blur-[190px] vqm-blob2" style={{ background: 'radial-gradient(circle, rgba(139,92,246,0.12), transparent 62%)' }} />
        <div className="absolute bottom-[-28%] left-[28%] w-[46vw] h-[46vw] rounded-full blur-[210px] vqm-blob" style={{ background: 'radial-gradient(circle, rgba(34,211,238,0.07), transparent 62%)' }} />
        <div className="absolute inset-0 vqm-grid opacity-[0.35]" />
        <div className="absolute inset-0" style={{ background: 'radial-gradient(95% 75% at 50% 40%, rgba(4,2,12,0) 0%, rgba(4,2,12,0.5) 100%)' }} />
        <div className="absolute inset-0 vqm-grain opacity-[0.3]" />
    </div>
);
    ...
    useEffect(() => {
        ...
        const resize = () => {
            ...
            parts = Array.from({ length: count }, () => ({
                ...
                a: Math.random() * 0.3 + 0.06, c: COLORS[(Math.random() * COLORS.length) | 0], tw: Math.random() * Math.PI * 2,
            }));
        };
        const draw = () => {
            if (!running) return;
...
const MkSpotlight = () => {
    ...
    return (
        <div className="fixed inset-0 pointer-events-none z-[5] hidden md:block">
            <div ref={ref} className="absolute -left-[300px] -top-[300px] w-[600px] h-[600px] rounded-full" style={{ background: 'radial-gradient(circle, rgba(129,140,248,0.06), transparent 60%)' }} />
        </div>
    );
};
...
export default function MarketingLayout({ children, title, description }) {
    ...
    return (
        <div className="min-h-screen bg-[#04020c] text-white font-sans selection:bg-indigo-500/40 overflow-x-hidden antialiased">
            <Head>
                    ...
                    <div className="hidden lg:flex items-center gap-1">
                        {navLinks.map(link => (
                            <Link key={link.href} href={link.href}
                                className={`relative px-5 py-2 text-[11px] font-bold uppercase tracking-[0.2em] transition-colors duration-300 rounded-full ${currentPath === link.href ? 'text-white bg-white/[0.06]' : 'text-slate-400 hover:text-white hover:bg-white/[0.04]'}`}>
                                {link.label}
                            </Link>
                        ))}
                    </div>
            ...
            {/* Footer */}
            <footer className="border-t border-white/[0.06] pt-24 pb-12 px-6 relative z-10 bg-[#04020c]">
                <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-12 mb-20">
                    <div className="md:col-span-5">
                        ...
                        <a href="https://wa.me/923091999489" className="inline-flex p-3 rounded-xl bg-white/5 border border-white/5 text-slate-500 hover:text-emerald-400 hover:border-emerald-500/20 hover:bg-emerald-500/5 transition-all duration-300">
                    ...
                    <div className="md:col-span-2">
                        ...
                        <ul className="space-y-3">
                            {['Features', 'Pricing', 'Blog', 'About'].map(l => (
                                <li key={l}><Link href={`/${l.toLowerCase()}`} className="text-sm text-slate-500 hover:text-white transition-colors font-medium">{l}</Link></li>
                            ))}
                        </ul>
                    </div>
                    <div className="md:col-span-2">
                        ...
                        <ul className="space-y-3">
                            {[{ label: 'Contact', href: '/contact' }, { label: 'Live Demo', href: '/demo' }, { label: 'Terms', href: '/terms' }, { label: 'Privacy', href: '/privacy' }].map(l => (
                                <li key={l.href}><Link href={l.href} className="text-sm text-slate-500 hover:text-white transition-colors font-medium">{l.label}</Link></li>
                            ))}
                        </ul>
                    </div>
                ...
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 pt-8 border-t border-white/[0.06]">
                    <span className="text-slate-600 text-[10px] font-black uppercase tracking-[0.2em]">© 2026 {appName}. All rights reserved.</span>
            ...
            <style>{`
                * { font-family: 'Inter','Figtree',system-ui,sans-serif; }
                ...
                .vq-grid-pattern { background-image: linear-gradient(rgba(255,255,255,0.02) 1px,transparent 1px), linear-gradient(90deg,rgba(255,255,255,0.02) 1px,transparent 1px); background-size:60px 60px; }
                .vq-dot-pattern { background-image: radial-gradient(rgba(255,255,255,0.05) 1px,transparent 1px); background-size:30px 30px; }
