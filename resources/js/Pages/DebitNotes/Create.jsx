...
const Create = ({ debitNote }) => {
    // Using local state to avoid mixing with Sales Tabs in Context
    const [currentInvoice, setCurrentInvoice] = useState({
        id: Date.now(),
        invoiceNumber: '',
        customer: null, // Acts as Supplier
        items: [{ id: Date.now(), product: null, quantity: 1, price: 0, discount: 0, discountType: 'fixed' }],
        paymentMethod: 'credit',
        amountPaid: 0, // Refund Amount
        discount: 0,
        tax: 0,
        delivery_charge: 0,
    ...
    useEffect(() => {
        if (debitNote) {
            const mappedState = {
                ...
                customer: debitNote.supplier, // Map Supplier to Customer field for compatibility
    ...
    // Mock Context functions to minimize code changes downstream
    const addInvoice = () => { };
    ...
    // Load initial products (unchanged)
    useEffect(() => {
    ...
    const processDebitNote = async (addToLedger = false, shouldPrint = false) => {
        // Stock Validation (Ensure we have stock to return)
        const validItems = currentInvoice.items.filter(item => item.product);
        // We generally allow returning whatever, but optionally check stock
        const allowOverselling = !shouldStopNegativeStock(settings);

        if (!allowOverselling) {
            for (const item of validItems) {
                let availableStock = item.product.stock_quantity || 0;
                // In Edit Mode, if we are updating a return, we might have already seemingly 'removed' it from stock? 
                // Actually in Debit Note, 'approved' reduces stock. 
                // If editing, we need to consider the original quantity we are effectively "putting back" before taking again.
                if (isEditMode && item.originalQuantity) {
                    // Logic: Current Stock + Original Return Qty = Max we can return now.
        ...
        try {
            const payload = {
                ...
                status: 'approved', // Default to approved/completed
                add_to_ledger: addToLedger, // Update Supplier Ledger
            ...
            if (response.data) {
                // Global Sync Trigger (Debit Notes affect stock)
    ...
    const initiateSave = (print = false) => {
        ...
        // Logic Review: Refund vs Ledger
        // Debit Note usually reduces Supplier Balance (Credit Ledger).
        // Or if 'Cash', it increases our Cash (Refund).
        // The API likely handles 'payment_method' logic.
        // We just need to pass 'add_to_ledger' if the user intends to update the Supplier Account.

        // Simple logic for now: Always Process
                    ...
                    <div className="w-80 bg-[#1a1d2e] flex flex-col overflow-hidden rounded-2xl shadow-2xl border border-slate-800">
                        ...
                        <div className="flex-1 p-3 space-y-3 overflow-y-auto hide-scrollbar">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    <label className="text-[9px] text-slate-500 font-bold uppercase block mb-1">Reference #</label>
                                    <input
                                        type="text"
                                    ...
                                    <button
                                        onClick={() => {
                                            ...
                                            showConfirm({
                                                ...
                                                onConfirm: () => {
                                                    // removeInvoice(currentInvoice.id); // mocked earlier
                                                    router.visit(route('store.debit-notes.index', { store_slug: store.slug }));
                                                }
                                            });
                                        }}
