            ...
            <div className="flex items-center justify-between mb-4 z-10">
                <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-1.5 text-xs">
                        ...
                        <span className="font-bold text-indigo-600 dark:text-indigo-400">{(window.amdSettings?.currency_symbol || 'Rs')} {totalSales.toLocaleString()}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 text-xs">
                        ...
                        <span className="font-bold text-emerald-600 dark:text-emerald-400">{(window.amdSettings?.currency_symbol || 'Rs')} {totalProfit.toLocaleString()}</span>
                    </div>
                </div>
                                    ...
                                    return (
                                        <div style={{
                                            ...
                                            <p style={{ fontSize: '13px', fontWeight: 'bold', color: '#e2e8f0', margin: 0 }}>
                                                💰 Sales: <span style={{ color: '#6366f1' }}>{(window.amdSettings?.currency_symbol || 'Rs')} {Number(payload.find(p => p.dataKey === 'sales')?.value || 0).toLocaleString()}</span>
                                            </p>
                                            {/* Gross Profit second */}
                                            <p style={{ fontSize: '13px', fontWeight: 'bold', color: '#e2e8f0', margin: 0 }}>
                                                ✨ Gross Profit: <span style={{ color: '#10b981' }}>{(window.amdSettings?.currency_symbol || 'Rs')} {Number(payload.find(p => p.dataKey === 'profit')?.value || 0).toLocaleString()}</span>
                                            </p>
                                        </div>
                                    );
                                }
