...
export function resolvePreset(state: OrbState, size: number): Resolved {
  ...
  // Pick closest preset size (20 vs 64) if exact size not defined
  const baseSize = modePresets[size as OrbSize] 
    ? (size as OrbSize) 
    : (size <= 32 ? 20 : 64);
    
  const preset = modePresets[baseSize] || modePresets[64];
  let opts: ModeOpts = { ...BASE_PROFILES[mode] };
  
  // Scale dynamically if custom size passed
  const customScale = baseSize !== size ? (size / baseSize) : 1;
  const countMul = (preset.count ?? 1) * (customScale < 1 ? customScale : 1);
  const sizeMul = (preset.size ?? 1) * customScale;

  if (countMul !== 1) opts = scaleCounts(opts, countMul);
  if (sizeMul !== 1) opts = scaleRadii(opts, sizeMul);
