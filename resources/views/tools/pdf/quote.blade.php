...
<head>
    ...
    <title>{{ $meta['doc_type'] ?? 'Quotation' }} {{ $meta['quote_number'] ?? '' }}</title>
    <style>
        ...
        .quote-title { font-size: 24px; font-weight: bold; letter-spacing: 1px; text-transform: uppercase; }
        ...
        .totals-table .highlight td { background: #f4f4f8; padding: 2mm 1.5mm; font-weight: bold; }
        ...
        .signature-container { margin-top: 12mm; page-break-inside: avoid; }
        .signature-title { font-size: 10px; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px; color: #1c1c28; margin-bottom: 3mm; }
        .signature-table { width: 100%; margin-top: 4mm; }
        .signature-table td { vertical-align: bottom; width: 50%; padding-right: 5mm; }
        .signature-line { border-bottom: 1px solid #1c1c28; height: 10mm; margin-bottom: 1.5mm; }
        .signature-label { font-size: 8.5px; color: #6b6b7a; }
        ...
        @if($template === 'modern')
            ...
            .quote-title { color: {{ $meta['accent_color'] ?? '#4f46e5' }}; }
        ...
        @if($template === 'compact')
            ...
            .signature-container { margin-top: 8mm; }
        @endif
    </style>
</head>
<body>
    ...
    <table class="header">
        <tr>
            ...
            <td style="width:45%;" class="right">
                <div class="quote-title">{{ $meta['doc_type'] ?? 'QUOTATION' }}</div>
                <table class="meta-table" style="margin-left:auto; margin-top:3mm;">
                    <tr><td class="label right">{{ $meta['doc_type'] ?? 'Quote' }} #</td><td class="bold">{{ $meta['quote_number'] ?? '—' }}</td></tr>
                    <tr><td class="label right">Issue date</td><td>{{ $meta['issue_date'] ?? '—' }}</td></tr>
                    <tr><td class="label right">Valid until</td><td class="bold">{{ $meta['valid_until'] ?? '—' }}</td></tr>
                </table>
            </td>
        </tr>
    </table>
    ...
    <table class="totals-table">
        ...
        @endif
        <tr class="grand"><td>Total Amount</td><td class="right accent">{{ $symbol }}{{ number_format($total, 2) }}</td></tr>
        
        @if($depositAmount > 0)
            <tr><td class="muted">Required Deposit</td><td class="right">{{ $symbol }}{{ number_format($depositAmount, 2) }}</td></tr>
            <tr class="highlight"><td>Balance Due</td><td class="right">{{ $symbol }}{{ number_format($balanceDue, 2) }}</td></tr>
        @endif
    </table>
    ...
    @if(!empty($meta['notes']) || !empty($meta['terms']))
        <div class="notes">
            @if(!empty($meta['notes']))
                <div class="heading">Notes & Scope</div>
                <div>{{ $meta['notes'] }}</div>
            @endif
            @if(!empty($meta['terms']))
                <div class="heading" style="margin-top:3mm;">Terms & Conditions</div>
                <div>{{ $meta['terms'] }}</div>
            @endif
        </div>
    @endif
    ...
    <div class="signature-container">
        ...
        <div class="muted" style="font-size: 8.5px; margin-bottom: 2mm;">
            By signing below, the client accepts this {{ strtolower($meta['doc_type'] ?? 'quotation') }} and agrees to the terms outlined above.
        </div>
