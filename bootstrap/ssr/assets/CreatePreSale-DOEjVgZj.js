...
import { Info, X, Plus, ArrowLeft, Settings, User, Wallet, Zap, ScanBarcode, CreditCard, Banknote, ChevronRight, CheckCircle2, TrendingUp, Type, GripVertical, Trash2, Package, Save, ArrowLeftRight, Printer } from "lucide-react";
...
const CreatePreSale = ({ sale }) => {
  const {
    activePreSaleInvoices: activeInvoices,
    currentPreSaleId: currentInvoiceId,
    setCurrentPreSaleId: setCurrentInvoiceId,
    addPreSaleInvoice: addInvoice,
    removePreSaleInvoice: removeInvoice,
    updatePreSaleInvoice: updateInvoice
  } = useWorkspace();
  ...
  const refreshPreSaleItems = async () => {
    ...
    try {
      ...
      const newItems = currentInvoice.items.map((item) => {
        ...
        if (latest) {
          const isFinalized = isEditMode || ["ordered", "completed"].includes(currentInvoice.status);
          return {
            ...item,
            product: latest,
      ...
      console.error("Failed to refresh pre-sale items", error);
    }
  };
  useEffect(() => {
  ...
  const handleProductSubmit = async (data, onError) => {
    try {
      const url = productModalMode === "create" ? route("store.inventory.store", { store_slug: store.slug }) : editingProduct?.id ? route("store.inventory.update", [store.slug, editingProduct.id]) : "";
  ...
  const selectProduct = (product, itemId) => {
    if ((product.available_stock ?? product.stock_quantity ?? 0) <= 0) {
      if (!window.confirm(`Only ${product.available_stock || 0} units available. This order will be on backorder. Continue?`)) {
        return;
      }
    }
  ...
  if (!currentInvoice) return /* @__PURE__ */ jsx("div", { className: "p-10 text-center text-slate-500", children: "Loading Workspace..." });
  ...
  const processSale = async (addToLedger = false, shouldPrint = false) => {
    ...
    try {
      const payload = {
        ...
        order_date: currentInvoice.date || (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
      ...
      let response;
      if (isEditMode) {
        response = await axios.put(route("store.sales.orders.update", [store.slug, currentInvoice.id]), payload);
      } else {
        response = await axios.post(route("store.pre-sales.store", {
          store_slug: store.slug
        }), payload);
      }
      if (response.data.success || isEditMode) {
        ...
        if (isEditMode) {
          showAlert({ title: "Success", message: "Pre-Sale updated successfully.", type: "success" });
          if (shouldPrint) {
            window.open(route("store.sales.print", [store.slug, currentInvoice.id]), "_blank");
          }
          if (!shouldPrint) router.visit(route("store.pre-sales.index", {
            store_slug: store.slug
          }));
        } else {
          if (shouldPrint) {
      ...
      console.error("Pre-Sale Error:", error.response?.data || error);
      const errorData = error.response?.data;
      let errorMessage = "Failed to save pre-sale.";
      if (errorData?.errors) {
  ...
  return /* @__PURE__ */ jsxs(OneGlanceLayout, { title: isEditMode ? `Edit Pre-Sale #${editState?.invoiceNumber || ""}` : "New Pre-Sale", activeMenu: "Sales", fullScreen: false, hideHeader: true, noPadding: true, children: [
    /* @__PURE__ */ jsx(Head, { title: isEditMode ? "Edit Pre-Sale" : "New Pre-Sale" }),
    /* @__PURE__ */ jsxs("div", { className: `h-full flex-1 flex flex-col bg-slate-50 dark:bg-void-800 transition-all duration-500 ${isSeniorMode ? "text-[20px] senior-mode" : ""}`, children: [
      /* @__PURE__ */ jsx("style", { children: `
                ...
                ` }),
      /* @__PURE__ */ jsxs("div", { className: `flex-1 flex flex-col lg:flex-row gap-2 min-h-0 px-2 pb-0 pt-2 lg:overflow-hidden overflow-y-auto text-scale-${textSize} ${isReadOnly ? "pointer-events-none opacity-85 select-none" : ""}`, children: [
        /* @__PURE__ */ jsxs("div", { className: "flex-1 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-800 flex flex-col overflow-hidden min-h-[400px] lg:min-h-0", children: [
          isReadOnly && /* @__PURE__ */ jsxs("div", { className: "bg-indigo-600 text-white font-bold text-xs px-4 py-2.5 text-center flex items-center justify-center gap-2 shrink-0", children: [
            ...
            " This Pre-Sale has been converted to a Sale and is read-only."
          ] }),
            ...
            /* @__PURE__ */ jsxs("div", { className: "flex items-center justify-between w-full relative", children: [
              ...
              /* @__PURE__ */ jsxs(
                "button",
                {
                  ...
                  children: [
                    ...
                    /* @__PURE__ */ jsx("span", { className: "truncate", children: currentInvoice.customer?.name || `PreSale #${activeInvoices.findIndex((inv) => inv.id === currentInvoice.id) + 1}` })
                  ]
                }
              ),
              /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-1.5 shrink-0", children: [
                ...
                /* @__PURE__ */ jsx(
                  ...
                  {
                    onClick: () => {
                      if (window.confirm("Are you sure you want to cancel and discard this pre-sale?")) {
          ...
          /* @__PURE__ */ jsxs("div", { className: "hidden lg:flex px-3 py-2 border-b border-slate-100 dark:border-slate-800 items-center gap-3 bg-slate-50/50 dark:bg-slate-800/30 shrink-0", children: [
            ...
            /* @__PURE__ */ jsxs("div", { className: "relative flex-1 max-w-xl", children: [
              /* @__PURE__ */ jsx("div", { className: "absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 z-10 pointer-events-none", children: /* @__PURE__ */ jsx(User, { size: 18 }) }),
              currentInvoice.customer ? /* @__PURE__ */ jsxs("div", { className: "relative", children: [
                ...
                /* @__PURE__ */ jsx(
                  AsyncPartyCombobox,
                  {
                    ...
                    inputClassName: customerError ? "border-red-500 ring-red-500/20" : ""
                  }
                ),
          ...
          /* @__PURE__ */ jsxs("div", { className: "p-3 bg-slate-900 space-y-2 shrink-0 border-t border-slate-800", children: [
            ...
            /* @__PURE__ */ jsxs("div", { className: "grid grid-cols-2 gap-2", children: [
              ...
              /* @__PURE__ */ jsxs(
                ...
                {
                  onClick: async () => {
                    if (!currentInvoice.id || !isEditMode) {
                      showAlert({ title: "Save First", message: "Please save this pre-sale first before converting.", type: "warning" });
                      return;
                    }
                    try {
                      const response = await axios.post(route("store.pre-sales.convert", [store.slug, currentInvoice.id]));
                      if (response.data.success) {
                        showAlert({
                          title: "Conversion Successful!",
                          message: "Pre-sale has been converted successfully to the main sale.",
                          type: "success"
                        });
                        removeInvoice(currentInvoice.id);
                  ...
                  className: `py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all active:scale-95 ${isEditMode && !isReadOnly ? "bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 text-white shadow-lg shadow-purple-500/20" : "bg-slate-700 text-slate-500 cursor-not-allowed"}`,
              ...
              /* @__PURE__ */ jsxs(
                "button",
                {
                  onClick: () => {
                    ...
                    showConfirm({
                      title: "Cancel Pre-Sale?",
                      message: "Discard this pre-sale? Items will be lost.",
                      type: "warning",
                  ...
                  className: "py-3 bg-red-500/10 hover:bg-red-500 hover:text-white text-red-500 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all border border-red-500/20 active:scale-95",
                  children: [
    ...
    showProfit && !showProfitModal && /* @__PURE__ */ jsx("div", { className: "fixed bottom-4 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-4 duration-200", children: /* @__PURE__ */ jsxs("div", { className: "bg-slate-900/95 backdrop-blur-lg rounded-2xl px-8 py-4 shadow-2xl border border-slate-700 flex items-center gap-6", children: [
      ...
      /* @__PURE__ */ jsx("p", { className: "text-xs text-slate-500 italic", children: "â†“ Drag down for details" })
    ] }) }),
    /* @__PURE__ */ jsx(
      FormModal,
      {
