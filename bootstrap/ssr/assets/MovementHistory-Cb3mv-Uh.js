...
function MovementHistory({ movements = [], filters = {}, products = [], warehouses = [] }) {
  ...
  const getTypeStyle = (type) => {
    switch (type) {
      case "sale":
        return { bg: "bg-brand-100 dark:bg-brand-900/30", text: "text-brand-600 dark:text-brand-400", icon: /* @__PURE__ */ jsx(TrendingUp, { size: 14 }), label: "Sale" };
      case "purchase":
      ...
      default:
        return { bg: "bg-sunken", text: "text-ink-secondary", icon: /* @__PURE__ */ jsx(Activity, { size: 14 }), label: type?.replace("_", " ") || "Unknown" };
    }
  };
  const columns = [
    {
      ...
      render: (row) => /* @__PURE__ */ jsxs("div", { className: "flex flex-col", children: [
        /* @__PURE__ */ jsx("span", { className: "font-bold text-ink-secondary dark:text-ink text-sm", children: new Date(row.created_at).toLocaleDateString() }),
        /* @__PURE__ */ jsx("span", { className: "text-1xs text-ink-muted font-mono", children: new Date(row.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) })
      ] })
    },
    {
      key: "product",
      ...
      render: (row) => /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-3 py-1", children: [
        /* @__PURE__ */ jsx("div", { className: "w-9 h-9 rounded-lg bg-app flex items-center justify-center shrink-0 border border-line overflow-hidden", children: row.product?.image ? /* @__PURE__ */ jsx("img", { src: `/storage/${row.product.image}`, className: "w-full h-full object-cover", alt: "", onError: (e) => e.target.style.display = "none" }) : /* @__PURE__ */ jsx(Package, { size: 16, className: "text-ink-muted" }) }),
        /* @__PURE__ */ jsxs("div", { children: [
          /* @__PURE__ */ jsx("div", { className: "font-bold text-sm text-ink line-clamp-1", title: row.product?.name, children: row.product?.name || "Unknown Item" }),
          /* @__PURE__ */ jsx("div", { className: "text-2xs font-mono text-ink-muted", children: row.product?.sku || "NO-SKU" })
        ] })
      ] })
    },
    {
    ...
    },
    {
      key: "reference",
      label: "Reference",
      render: (row) => /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-2 text-ink-muted", children: [
    ...
    },
    {
      key: "warehouse",
      label: "Location",
      render: (row) => /* @__PURE__ */ jsx("span", { className: "text-xs font-medium text-ink-secondary bg-sunken px-2 py-1 rounded", children: row.warehouse?.name || "Main" })
    },
    {
      key: "user",
      label: "Authorized By",
      ...
      render: (row) => /* @__PURE__ */ jsxs("div", { className: "flex items-center justify-end gap-2", children: [
        /* @__PURE__ */ jsx("div", { className: "text-xs font-medium text-ink-secondary", children: row.user?.name || "System" }),
        /* @__PURE__ */ jsx("div", { className: "w-6 h-6 rounded-full bg-sunken flex items-center justify-center text-2xs font-bold text-ink-muted", children: row.user?.name ? row.user.name.charAt(0) : "S" })
      ] })
    }
  ];
  const filterDefs = [
