...
import { X, Plus, ArrowLeft, Settings, User, Wallet, CheckCircle2, Zap, ScanBarcode, CreditCard, Banknote, ChevronRight, TrendingUp, Type, GripVertical, Trash2, Package, Printer, ArrowLeftRight } from "lucide-react";
...
const Create = ({ debitNote }) => {
  ...
  const isEditMode = !!debitNote;
  const [editState, setEditState] = useState(null);
  useEffect(() => {
    if (debitNote) {
      const mappedState = {
        id: debitNote.id,
        invoiceNumber: debitNote.reference_number,
        customer: debitNote.supplier,
        // Map Supplier to Customer field for compatibility
      ...
      setCurrentInvoice(mappedState);
      setEditState(mappedState);
    }
  }, [debitNote]);
  const addInvoice = () => {
  };
  const removeInvoice = () => router.visit(route("store.debit-notes.index", { store_slug: store.slug }));
  const setCurrentInvoiceId = () => {
  };
  const currentInvoiceId = currentInvoice.id;
  const activeInvoices = [currentInvoice];
  ...
  const refreshDebitNoteItems = async () => {
    ...
    try {
      ...
      console.error("Failed to refresh debit note items", error);
    }
  };
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const response = await axios.get(route("store.suppliers.search", { store_slug: store.slug }), { params: { search: customerSearch } });
  ...
  const selectProduct = (product, itemId) => {
    const updatedItems = currentInvoice.items.map(
      (item) => item.id === itemId ? {
        ...item,
        product,
        // Default to COST price for Debit Note (Return to Supplier)
  ...
  const processDebitNote = async (addToLedger = false, shouldPrint = false) => {
      ...
      for (const item of validItems) {
        ...
        if (item.quantity > availableStock) {
          showAlert({
            ...
            message: `You cannot return ${item.quantity} units of "${item.product.name}" because you only have ${availableStock} in stock.`,
            type: "error"
          });
          return;
        }
    ...
    try {
      const payload = {
        supplier_id: currentInvoice.customer.id,
        // Mapped from Customer state
        ...
        reference_number: currentInvoice.invoiceNumber,
        // Optional manual ref
        date: currentInvoice.date,
        status: "approved",
        // Default to approved/completed
      ...
      if (response.data) {
        ...
        const newId = isEditMode ? currentInvoice.id : response.data.id;
        setLastSaleId(newId);
        if (isEditMode) {
  ...
  return /* @__PURE__ */ jsxs(OneGlanceLayout, { title: isEditMode ? `Edit Debit Note #${editState?.invoiceNumber || ""}` : "New Debit Note", activeMenu: "Returns", fullScreen: false, hideHeader: true, noPadding: true, children: [
    /* @__PURE__ */ jsx(Head, { title: isEditMode ? "Edit Debit Note" : "New Debit Note" }),
            ...
            activeInvoices.map((inv, idx) => /* @__PURE__ */ jsxs(
              "div",
              {
                ...
                children: [
                  ...
                  /* @__PURE__ */ jsx("span", { className: "text-xs font-bold truncate", children: inv.customer?.name || `Note #${idx + 1}` }),
                  /* @__PURE__ */ jsx(
                    "button",
                    {
                      onClick: (e) => {
                        ...
                        const proceed = () => {
                          ...
                          if (activeInvoices.length === 1) router.visit(route("store.debit-notes.index", { store_slug: store.slug }));
                        };
              ...
              /* @__PURE__ */ jsxs(
                "button",
                {
                  ...
                  children: [
                    ...
                    /* @__PURE__ */ jsx("span", { className: "truncate", children: currentInvoice.customer?.name || `Note #${activeInvoices.findIndex((inv) => inv.id === currentInvoice.id) + 1}` })
                  ]
                }
              ),
          ...
          /* @__PURE__ */ jsxs("div", { className: "hidden lg:flex px-3 py-2 border-b border-slate-100 dark:border-slate-800 items-center justify-between gap-3 bg-slate-50/50 dark:bg-slate-800/30 shrink-0", children: [
            ...
            /* @__PURE__ */ jsxs("div", { className: "relative flex-1 max-w-xl", children: [
              ...
              currentInvoice.customer ? /* @__PURE__ */ jsxs("div", { className: "relative", children: [
                ...
                customerError && /* @__PURE__ */ jsx("p", { className: "absolute -bottom-5 left-2 text-[10px] font-bold text-red-500 animate-pulse", children: "Please select a registered supplier" })
              ] })
            ] }),
            /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-2", children: [
        ...
        /* @__PURE__ */ jsxs("div", { className: "hidden lg:flex w-full lg:w-80 bg-[#1a1d2e] flex-col overflow-hidden rounded-2xl shadow-2xl border border-slate-800 shrink-0", children: [
          /* @__PURE__ */ jsx("div", { className: "p-4 border-b border-slate-800/50 bg-slate-900/30 shrink-0", children: currentInvoice.customer ? /* @__PURE__ */ jsxs("div", { className: "space-y-3", children: [
            ...
            /* @__PURE__ */ jsxs("div", { className: `space-y-1 bg-slate-800/30 rounded-lg p-2 ${textSize >= 3 ? "text-sm" : "text-xs"}`, children: [
              /* @__PURE__ */ jsxs("div", { className: "flex justify-between items-center", children: [
                /* @__PURE__ */ jsx("span", { className: "text-slate-500 font-medium", children: "Supplier Balance:" }),
          ...
          /* @__PURE__ */ jsxs("div", { className: "flex-1 p-3 space-y-3 overflow-y-auto hide-scrollbar", children: [
            /* @__PURE__ */ jsxs("div", { className: "grid grid-cols-2 gap-2", children: [
              /* @__PURE__ */ jsxs("div", { children: [
                /* @__PURE__ */ jsx("label", { className: "text-[9px] text-slate-500 font-bold uppercase block mb-1", children: "Reference #" }),
                /* @__PURE__ */ jsx(
                  "input",
                  {
                    type: "text",
            ...
            /* @__PURE__ */ jsx("div", { className: "space-y-2 pt-3 border-t border-slate-800/50", children: /* @__PURE__ */ jsxs("div", { className: "flex justify-between items-center", children: [
            ...
            /* @__PURE__ */ jsxs("div", { className: "flex items-center justify-between bg-emerald-900/20 rounded-xl p-3 border border-emerald-800/30", children: [
              /* @__PURE__ */ jsx("span", { className: "text-xs text-emerald-400 font-bold", children: "Refund Received" }),
              /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-2", children: [
            ...
            /* @__PURE__ */ jsxs("div", { className: `flex items-center justify-between rounded-xl p-3 border ${balanceDue > 0 ? "bg-indigo-900/20 border-indigo-800/30" : "bg-slate-900/20 border-slate-800/30"}`, children: [
              /* @__PURE__ */ jsx("span", { className: `text-xs font-bold ${balanceDue > 0 ? "text-indigo-400" : "text-slate-400"}`, children: "Net Credited" }),
              /* @__PURE__ */ jsx("span", { className: `font-bold text-base ${balanceDue > 0 ? "text-indigo-400" : "text-slate-400"}`, children: formatCurrency(balanceDue, store) })
            ] })
          ] }),
          /* @__PURE__ */ jsxs("div", { className: "p-3 bg-slate-900 space-y-2 shrink-0 border-t border-slate-800", children: [
            /* @__PURE__ */ jsxs("div", { className: "flex justify-between items-center", children: [
              /* @__PURE__ */ jsx("span", { className: "text-[10px] text-slate-500 font-bold uppercase", children: "Total Return Value" }),
                ...
                /* @__PURE__ */ jsxs(
                  "button",
                  {
                    onClick: () => {
                      ...
                      showConfirm({
                        title: "Cancel Note?",
                        message: "Discard this debit note? Items will be lost.",
                        type: "warning",
      ...
      /* @__PURE__ */ jsx("div", { className: "fixed inset-0 flex items-center justify-center z-[210] p-4", children: /* @__PURE__ */ jsxs("div", { className: "bg-white dark:bg-[#1a1d2e] rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-200 dark:border-slate-700/50", children: [
        ...
        /* @__PURE__ */ jsxs("div", { className: "p-6 space-y-5 bg-gradient-to-b from-white to-slate-50 dark:from-[#1a1d2e] dark:to-[#0f121d]", children: [
          /* @__PURE__ */ jsxs("div", { className: "text-center py-2", children: [
            ...
            /* @__PURE__ */ jsx("p", { className: "text-5xl font-black bg-gradient-to-r from-emerald-500 to-teal-500 dark:from-emerald-400 dark:to-teal-400 bg-clip-text text-transparent", children: formatCurrency(overpaymentDetails.amount, store) }),
