 ...
 */
class CodeStackingTest extends TestCase
{
    ...
    public function second_code_upgrades_license_to_ltd_2_and_updates_tenant(): void
    {
        ...
        $tenant->refresh();
        $this->assertEquals('ltd_2', $tenant->plan, 'Tenant plan was not upgraded to ltd_2.');
    ...
    public function third_code_upgrades_to_ltd_3_with_correct_unlimited_limits(): void
    {
        ...
        $tenant->refresh();
        $this->assertEquals('ltd_3', $tenant->plan, 'Tenant plan was not upgraded to ltd_3.');
    ...
    public function transaction_limit_blocks_write_at_threshold(): void
    {
        ...
        $response = $this->actingAs($user)
            ...
            ->postJson(route('store.v3.sales.store', ['store_slug' => $tenant->slug]), [
                ...
                'amount_received'=> 100,
                'items'          => [
                    ['product_id' => 1, 'qty' => 1, 'unit_price' => 100, 'sale_uom' => 'unit'],
                ],
            ]);
    ...
    public function ltd_1_user_cannot_create_second_store(): void
    {
        ...
        $response->assertStatus(422); // back()->withErrors() triggers a 422 on JSON requests
        $this->assertStringContainsString(
            'maximum of 1 store',
            strtolower($response->json('errors.name.0') ?? ''),
    ...
    public function ltd_2_user_can_create_second_store_but_not_third(): void
    {
        ...
        $thirdStore->assertStatus(422);
        $this->assertStringContainsString(
            'maximum of 2 store',
            strtolower($thirdStore->json('errors.name.0') ?? ''),
    ...
    public function read_access_is_never_blocked_at_limit(): void
    {
        ...
        DB::table('sales')->insert(
            array_fill(0, $limit, [
                'id'               => \Illuminate\Support\Str::uuid(),
                'tenant_id'        => $tenant->id,
                'reference_number' => 'INV-READ-TEST',
                'status'           => 'posted',
                'sale_date'        => now()->toDateString(),
                'created_at'       => now(),
                'updated_at'       => now(),
