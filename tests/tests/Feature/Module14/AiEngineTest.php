...
test('resolves tenant context for Vena Assist chatbot widget', function () {
    ...
    // The platform-wide knowledge base is never exposed to a PUBLIC caller
    // (VenaAssistController step 3), so a visitor-widget call gets no KB matches
    // and therefore 'low' confidence. Agents (authenticated) get the KB.
    $response->assertJsonPath('similar_kb', []);
