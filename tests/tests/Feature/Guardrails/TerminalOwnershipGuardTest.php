...
class TerminalOwnershipGuardTest extends VenQoreTestCase
{
    ...
    {
        parent::setUp();
        // Exercise the endpoint logic; the production default is OFF (503).
        config(['venqore.terminal_telemetry_enabled' => true]);
    }

    public function test_telemetry_is_disabled_by_default_config(): void
    {
        config(['venqore.terminal_telemetry_enabled' => false]);
        $this->postJson('/api/terminal/activities', ['device_id' => 'x', 'activities' => []])
            ->assertStatus(503);
    }
    ...
    public function test_unauthenticated_caller_cannot_hijack_another_tenants_terminal(): void
    {
        ...
        // The endpoint should reject the attempt outright. Since SEC-04
        // (2026-09-10) an unknown device is refused before any store lookup (401).
        $this->assertContains($response->status(), [401, 403]);
    }

    /**
     * SEC-04 (2026-09-10): activity submission must NEVER claim a terminal.
     * Claiming happens only via /api/heartbeat with a valid pairing token
     * (see HeartbeatOwnershipGuardTest).
     */
    public function test_activities_cannot_claim_an_unclaimed_terminal(): void
    {
        app()->forgetInstance('current.tenant');
        ...
        $fresh = Terminal::withoutGlobalScope('tenant')->find($terminal->id);
        $this->assertNull($fresh->tenant_id, 'Activity submission must not claim a terminal.');
    }
