...
class InventoryService
{
    ...
    public function receivePurchase(string $purchaseId): void
    {
        ...
        foreach ($items as $item) {
            $prodType = DB::table('products')->where('tenant_id', $tid)->where('id', $item->product_id)->value('type');
            if ($prodType === 'service') {
                continue;
            }
     ...
     * This acts as a drop-in replacement for the legacy InventoryService::processSale.
     */
    public function processSale(array $items, $partyId = null)
    {
        return DB::transaction(function () use ($items, $partyId) {
            ...
            if (!$warehouseId) {
                throw new \RuntimeException("No active warehouse configured for tenant {$this->tenantId}.");
            }

            foreach ($items as $item) {
                $productId = $item['id'] ?? $item['product_id'];
                $quantity = (float) ($item['quantity'] ?? $item['qty']);
                ...
                // Deduct stock using FifoService (FIFO batches)
                $this->fifo->deductStock($productId, $warehouseId, $quantity);

                // Update physical stock in stocks table
