...
class RecalculateAccountBalances extends Command
{
    ...
    private function recalculateForTenant(int $tenantId): void
    {
        ...
        DB::transaction(function () use ($tenantId) {
            // Scope accounts to this tenant only
            $accounts = Account::where('tenant_id', $tenantId)->lockForUpdate()->get();
            $count = 0;

            foreach ($accounts as $account) {
                // Scope journal items to non-reversed entries for this tenant
