...
class SaleObserver
{
    ...
    public function updating(Sale $sale): void
    {
        ...
        if ($sale->getOriginal('status') === 'posted') {
            ...
            foreach (self::IMMUTABLE_COLUMNS as $col) {
                if ($sale->isDirty($col)) {
                    throw new \Exception("Accounting Safety Lock: Cannot modify financial data on a 'posted' sale. Please use the Return/Credit Note flow for corrections.");
                }
            }
        }
    }
    ...
    public function deleting(Sale $sale): void
    {
        if ($sale->status === 'posted') {
            throw new \Exception("Accounting Safety Lock: Posted sales cannot be deleted. This is an authoritative financial document. Use the Return flow to reverse stock and income.");
        }
    }
}
