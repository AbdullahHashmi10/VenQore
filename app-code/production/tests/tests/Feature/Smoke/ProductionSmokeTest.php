...
test('[SMOKE-07] registration sends guests to the builder', function () {
    // /register is a hand-off to /build-workspace (the one signup path).
    $response = $this->get('/register');

    expect($response->status())->toBe(302);
    expect($response->headers->get('Location'))->toContain('/build-workspace');

    expect($this->get('/build-workspace')->status())->toBe(200);
});
