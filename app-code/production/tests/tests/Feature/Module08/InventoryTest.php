...
test('stock transfer between warehouses preserves total stock', function () {
    ...
    $warehouseA = Warehouse::firstOrCreate(['tenant_id' => $tenant->id], ['name' => 'Warehouse A', 'code' => 'W-A']);
...
test('stock take records discrepancy and adjusts stock', function () {
    ...
    $warehouse = Warehouse::firstOrCreate(['tenant_id' => $tenant->id], ['name' => 'Main', 'code' => 'MAIN']);
...
test('v3_stock_transfers_enforce_tenant_isolation_and_update_physical_stock', function () {
    ...
    $warehouseA = Warehouse::firstOrCreate(['tenant_id' => $tenant->id], ['name' => 'A', 'code' => 'W-A']);
...
test('prevents v3 stock transfer from reading or mutating another tenants batches', function () {
    $tenantA = $this->createTenant(null, 'scale');
    $tenantB = $this->createTenant(null, 'scale');

    $this->actingAsOwner($tenantA);
    $this->seedTenantDefaults($tenantA);

    $warehouseA = Warehouse::firstOrCreate(['tenant_id' => $tenantA->id], ['name' => 'A', 'code' => 'W-A']);
