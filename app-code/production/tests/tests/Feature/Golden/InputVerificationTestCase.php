...
abstract class InputVerificationTestCase extends VenQoreTestCase
{
    ...
    protected function assertJournalLine(
        ...
        $actualAmount = $lines->where('code', $accountCode)->sum($side);
        
        ClaimLogger::log(new VerificationClaim(
            expectedValue: $expectedAmount,
            actualValue: $actualAmount,
            metric: $message ?: "Journal Line {$accountCode} {$side}",
            surface: 'InputVerificationTestCase'
        ));
    ...
    protected function assertJournalBalanced(\Illuminate\Support\Collection $lines, string $context = ''): void
    {
        ...
        ClaimLogger::log(new VerificationClaim(
            expectedValue: $dr,
            actualValue: $cr,
            metric: "[Ledger Health] Journal Balanced" . ($context ? " {$context}" : ''),
            surface: 'Ledger Invariant Suite'
        ));
    ...
    protected function assertLedgerInvariantsHold(string $context = ''): void
    {
        ...
        ClaimLogger::log(new VerificationClaim(
            expectedValue: 0,
            actualValue: $unbalanced,
            metric: "[Ledger Health] No Unbalanced Entries" . ($context ? " {$context}" : ''),
            surface: 'Ledger Invariant Suite'
        ));
        ...
        ClaimLogger::log(new VerificationClaim(
            expectedValue: $fifo,
            actualValue: $gl1100,
            metric: "[Ledger Health] GL 1100 matches FIFO Valuation" . ($context ? " {$context}" : ''),
            surface: 'Ledger Invariant Suite'
        ));

        $this->assertEqualsWithDelta($fifo, $gl1100, $this->TOLERANCE,
            "Ledger invariant broken{$context}: GL 1100 (Rs.{$gl1100}) does not match FIFO valuation (Rs.{$fifo})"
        );
