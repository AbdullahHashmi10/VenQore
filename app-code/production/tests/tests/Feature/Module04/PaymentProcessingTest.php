...
test('routes sale checkout cash overpayment to Customer Advances Account 2060 in double-entry ledgers', function () {
    $customer = Party::factory()->create(['tenant_id' => $this->tenant->id, 'type' => 'customer']);
    $product = Product::factory()->create(['tenant_id' => $this->tenant->id, 'price' => 1000.00]);
    Stock::updateOrCreate(['product_id' => $product->id, 'warehouse_id' => $this->warehouseId], ['quantity' => 10]);
    ...
    // Verify double-entry ledger has PKR 200 credit on Customer Advances (Account 2060).
    // 2100 is Sales Tax Payable in this chart (TenantDefaultSeeder / TaxService) —
    // an untaxed sale's surplus must never land there.
    $this->assertJournalEntry([
        'tenant_id' => $this->tenant->id,
        'account_code' => '2060', // Customer Advances
        'credit' => 200.00,
    ]);
    $taxLines = DB::table('journal_items as ji')
        ->join('accounts as a', 'a.id', '=', 'ji.account_id')
        ->where('ji.tenant_id', $this->tenant->id)->where('a.code', '2100')->count();
    expect($taxLines)->toBe(0);
});
...
test('blocks concurrent checkout requests that attempt to exceed the monthly transaction limit', function () {
    ...
    $this->assertTrue(
        in_array($response2->status(), [403, 422]) ||
        $response2->exception instanceof PlanLimitException
    );
});
