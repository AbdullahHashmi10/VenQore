...
class SuiteIntegrityTest extends TestCase
{
     ...
     *
     * LAYOUT FIX 2026-09-10. The suite no longer lives in a separate Tester/
     * folder: the tester root is <project>/tests (phpunit.xml, bootstrap.php,
     * Scripts/, VerificationCenter/ …) and the live tree is <project>/tests/tests.
     * The guard used to look only at Tester/Golden/tests and SKIP when that
     * folder was absent — i.e. it never ran. It now scans the whole tester root:
     * any *Test.php outside the live tree (other than a deliberate _archive/
     * quarantine) is stranded, plus the legacy archive locations if present.
     * It found two real strays on its first run (tests/Unit/*Test.php — no
     * phpunit.xml ever loaded them); they were moved into tests/tests/Unit/AiBuilder.
     */
    public function test_no_test_files_hide_outside_the_live_tree(): void
    {
        $testerRoot = $this->normalize($this->testerRoot());
        $liveTree   = $testerRoot . '/tests';

        // Sanity: the scan roots must be the real ones, or the guard is vacuous.
        $this->assertDirectoryExists($liveTree, 'Live test tree not found — path logic is wrong.');
        $this->assertFileExists(__FILE__);
        $this->assertStringStartsWith($liveTree . '/', $this->normalize(__FILE__), 'This guard must itself live in the live tree.');

        $stranded = [];

        // 1. Anything under the tester root but outside the live tree. A *Test.php
        //    there is invisible to every phpunit.xml yet looks like real coverage
        //    to a human reading the directory.
        foreach ($this->rglob($testerRoot . '/*Test.php') as $file) {
            $norm = $this->normalize($file);
            if (strpos($norm, $liveTree . '/') === 0) {
                continue; // live tree — covered by test_every_test_file_maps_to_a_registered_suite()
            }
            if (strpos($norm, '/_archive/') !== false) {
                continue; // deliberate, documented quarantine
            }
            $stranded[] = $norm;
        }

        // 2. Legacy archive trees from the old Tester/ layout. Nothing executable
        //    may live there; absent is fine (and is the normal state here).
        foreach ([
            $testerRoot . '/Golden/tests',
            $this->projectRoot() . '/Tester/Golden/tests',
            $this->projectRoot() . '/Tester/tests',
        ] as $archive) {
            if (is_dir($archive)) {
                foreach ($this->rglob($archive . '/*Test.php') as $file) {
                    $stranded[] = $this->normalize($file);
                }
            }
        }

        $stranded = array_values(array_unique($stranded));
        sort($stranded);

        $this->assertSame(
            [],
            $stranded,
            "Test files are parked outside the live tree where no suite can load them.\n"
                . "Move them into tests/tests/ (the source suite), into an _archive/ folder if they are\n"
                . "deliberately retired, or delete them outright."
        );

        // And the two trees must agree. FinalTester/tests (old layout only) is a
        // materialised view of the source suite; if it has drifted, the dashboard
        // is reporting on code that is not the code under source control.
        $source = $liveTree;
        ...
        if (is_dir($mirror)) {
            ...
            $this->assertSame(
                [],
                $missing,
                "These test files exist in tests/tests but NOT in FinalTester/tests, so the "
