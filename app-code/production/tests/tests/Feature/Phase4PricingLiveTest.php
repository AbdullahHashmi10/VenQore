...
class Phase4PricingLiveTest extends TestCase
{
    ...
    public function it_loads_single_source_of_truth_pricing_config()
    {
        ...
        $this->assertArrayHasKey('solo', $pricing['plans']);
        $this->assertArrayHasKey('starter', $pricing['plans']);
        $this->assertArrayHasKey('core', $pricing['plans']);
        $this->assertArrayHasKey('scale', $pricing['plans']);

        $this->assertEquals(0.00, $pricing['plans']['solo']['price_monthly']);
        $this->assertEquals(49.00, $pricing['plans']['starter']['price_monthly']);
        $this->assertEquals(99.00, $pricing['plans']['core']['price_monthly']);
        $this->assertEquals(299.00, $pricing['plans']['scale']['price_monthly']);

        $this->assertArrayHasKey('quantity_addons', $pricing);
    }
    ...
    public function it_executes_v4_tenant_migration_command()
    {
        ...
        $this->assertEquals('solo', $legacyTenant->fresh()->plan);
    }
