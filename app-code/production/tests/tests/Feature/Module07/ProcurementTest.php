...
test('fifo_batch_creation_on_purchase', function () {
    ...
    $warehouse = \App\Models\Warehouse::firstOrCreate(['tenant_id' => $tenant->id], ['name' => 'Main Warehouse']);
...
test('partial_receiving_updates_order_status_and_stock_correctly', function () {
    ...
    $this->assertEquals('pending', $purchase->workflow_status);
    ...
    // Assert received_qty=30 on purchase item
    $item = PurchaseItem::find($purchaseItem->id);
    ...
    $this->assertEquals('partial', $purchase->fresh()->workflow_status);
});
...
test('fifo_batch_poisoning_prevented_on_negative_or_zero_values', function () {
    ...
    $warehouse = \App\Models\Warehouse::firstOrCreate(['tenant_id' => $tenant->id], ['name' => 'Main']);
