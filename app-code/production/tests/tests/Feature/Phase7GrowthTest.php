...
class Phase7GrowthTest extends TestCase
{
    ...
    {
        Config::set('smartcapture.shared_catalog_threshold', 3);

        $tenant2 = Tenant::create(['name' => 'Store 2', 'slug' => 'store-2', 'plan' => 'starter', 'status' => 'active']);
        $tenant3 = Tenant::create(['name' => 'Store 3', 'slug' => 'store-3', 'plan' => 'starter', 'status' => 'active']);
    ...
    public function it_wires_shared_catalog_into_learning_service_and_fuzzy_match_strategy_six()
    {
        ...
        $tenant2 = Tenant::create(['name' => 'Store 2B', 'slug' => 'store-2b', 'plan' => 'starter', 'status' => 'active']);
        $tenant3 = Tenant::create(['name' => 'Store 3B', 'slug' => 'store-3b', 'plan' => 'starter', 'status' => 'active']);
        ...
        $product2 = Product::create(['tenant_id' => $tenant2->id, 'name' => 'Universal Cola 500ml', 'sku' => 'COLA-500-2', 'price' => 2.50]);
        $product3 = Product::create(['tenant_id' => $tenant3->id, 'name' => 'Universal Cola 500ml', 'sku' => 'COLA-500-3', 'price' => 2.50]);
        ...
        // Confirm from 2 more tenants to reach threshold 3
        app()->instance('current.tenant', $tenant2);
        $learning->remember('product', 'Universal Cola 500ml', (string) $product2->id, 'Universal Cola 500ml');

        app()->instance('current.tenant', $tenant3);
        $learning->remember('product', 'Universal Cola 500ml', (string) $product3->id, 'Universal Cola 500ml');

        app()->instance('current.tenant', $this->tenant);
        $sp->refresh();
        $this->assertTrue((bool) $sp->is_published);
    ...
    public function it_enforces_public_tool_daily_budget_cap_atomically()
    {
        ...
        // The global anonymous ceiling is owned by AiGateway::meter() under
        // the '{feature}:global' spend scope (ai_limits.features.public_tool).
        DB::table('ai_spend_counters')->insert([
            'scope'     => 'public_tool:global',
            'day'       => $todayStr,
            'spend_usd' => 10.00,
            'cap_usd'   => 10.00,
            'tripped'   => true,
