    ...
    public function up(): void
    {
        ...
        if (in_array($driver, ['mysql', 'mariadb'], true)) {
            ...
            if (Schema::hasTable('stock_movements') && Schema::hasColumn('stock_movements', 'product_id')) {
                try { DB::statement('ALTER TABLE `stock_movements` DROP FOREIGN KEY `stock_movements_product_id_foreign`'); } catch (\Throwable $e) {}
                try { DB::statement('ALTER TABLE `stock_movements` DROP FOREIGN KEY `stock_movements_product_id_foreign_restrict`'); } catch (\Throwable $e) {}
                try {
                    DB::statement(
            ...
            if (Schema::hasTable('manufacturing_logs') && Schema::hasColumn('manufacturing_logs', 'sale_id')) {
                try { DB::statement('ALTER TABLE `manufacturing_logs` DROP FOREIGN KEY `manufacturing_logs_sale_id_foreign`'); } catch (\Throwable $e) {}
                try { DB::statement('ALTER TABLE `manufacturing_logs` DROP FOREIGN KEY `manufacturing_logs_sale_id_foreign_restrict`'); } catch (\Throwable $e) {}
                try {
                    DB::statement(
