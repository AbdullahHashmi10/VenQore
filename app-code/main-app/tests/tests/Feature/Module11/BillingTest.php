...
test('two_codes_stacked_upgrades_to_ltd2', function () {
    ...
    $this->assertNotNull($tenant->plan_limits);
    $ltd2Limits = \App\Services\PlanRepository::getLtdSnapshot('ltd_2');
    $this->assertNotEmpty($ltd2Limits, 'plans.ltd_2 config must exist');
...
test('subscription_updated_webhook_updates_tenant_plan', function () {
    ...
    $tenant->refresh();
    $this->assertEquals('core', $tenant->plan);
