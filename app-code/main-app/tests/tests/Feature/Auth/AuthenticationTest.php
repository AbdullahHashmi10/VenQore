...
class AuthenticationTest extends VenQoreTestCase
{
    ...
    public function test_users_can_authenticate_using_the_login_screen(): void
    {
        ...
        // AUTH-01: the password alone never grants a session.
        $this->assertGuest();
        $response->assertRedirect(route('otp.show', absolute: false));

        $code = null;
        \Illuminate\Support\Facades\Mail::assertQueued(\App\Mail\EmailOtpCodeMail::class, function ($mail) use (&$code, $user) {
            $code = $mail->code;
    ...
    public function test_csrf_token_mismatch_exception_for_inertia_requests(): void
    {
        ...
        $response = $this->from('/_test/csrf-mismatch')->withSession([])->post('/_test/csrf-mismatch', [], [
            'X-Inertia' => 'true',
        ]);

        $response->assertStatus(409);
        $response->assertHeader('X-Inertia-Location', '/_test/csrf-mismatch');
