...
class RegistryDriftTest extends TestCase
{
    ...
    public function test_every_live_test_file_is_registered(): void
    {
        ...
        foreach ($this->liveTestFiles() as $file) {
            // Relative to the suite root, not the FIRST "/tests/" in the absolute
            // path: in main-app the suite lives at tests/tests/, so that matched
            // the outer folder and every file looked unregistered.
            $root = str_replace('\\', '/', $this->testerRoot() . '/tests');
            $rel = 'tests/' . ltrim(substr(str_replace('\\', '/', $file), strlen($root)), '/');
