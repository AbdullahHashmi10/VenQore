...
test('ltd tier3 blocked after 5 stores', function () {
    $user = User::factory()->create([
        'last_store_id' => null,
    ]);
    ...
    // Create 5 existing stores owned by this user per V11 Part 7
    for ($i = 1; $i <= 5; $i++) {
    ...
    $errors = session('errors')->get('name');
    $this->assertStringContainsString('allows a maximum of 5 store', $errors[0]);
});
...
test('woocommerce webhook requires a valid signature', function () {
    $tenant = $this->createTenant('woo-webhook-sec', 'scale');
