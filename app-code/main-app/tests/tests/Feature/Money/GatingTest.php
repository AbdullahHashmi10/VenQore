...
test('M1-11: WooCommerce is blocked for Starter and Growth and allowed on Scale', function () {
    // Growth Tenant
    $growthTenant = $this->createTenant('growth-test-store', 'ltd_2'); // ltd_2 maps to growth/core
    $this->actingAsTenantUser($growthTenant, 'owner');

    // Assert PlanGate::check is false on Growth
    $this->assertFalse(PlanGate::check('woocommerce'));

    // Business/Scale Tenant has woocommerce included in V11
    $businessTenant = $this->createTenant('business-test-store', 'business');
    $this->actingAsTenantUser($businessTenant, 'owner');

    // Assert PlanGate::check is true on Scale/Business
    $this->assertTrue(PlanGate::check('woocommerce', $businessTenant));
});

test('M1-11: Scale-only features like white_label are blocked on Starter', function () {
    $starterTenant = $this->createTenant('starter-test-store', 'starter');
    $this->actingAsTenantUser($starterTenant, 'owner');

    // Assert PlanGate::check is false for Scale fence
    $this->assertFalse(PlanGate::check('white_label'));
    $this->assertFalse(PlanGate::check('multi_branch'));
});
...
test('Tenant featuresArray returns correct feature availability flags based on plan', function () {
    ...
    // 2. Starter Plan (V11: BOM and production are universal, e_invoicing and reminders are paid-only, white_label and multi_branch are scale-only)
    $starterTenant = $this->createTenant('starter-features-test', 'starter');
    $featuresStarter = $starterTenant->featuresArray();
    expect($featuresStarter['production'])->toBeTrue()
        ->and($featuresStarter['bill_of_materials'])->toBeTrue()
        ->and($featuresStarter['e_invoicing'])->toBeTrue()
        ->and($featuresStarter['invoice_reminders'])->toBeTrue()
        ->and($featuresStarter['white_label'])->toBeFalse()
        ->and($featuresStarter['multi_branch'])->toBeFalse();
});
