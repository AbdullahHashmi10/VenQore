...
class RegressionFixesTest extends VenQoreTestCase
{
    ...
    public function test_verify_elevated_pin_by_platform_user_and_membership(): void
    {
        ...
        // 2026-09-10 (security audit): the platform PIN is NOT an override for
        // a store's elevated actions any more — a store action is authorised by
        // a member of that store. Without a chosen member the request is refused.
    ...
    public function test_woocommerce_integration_is_gated_by_plan(): void
    {
        ...
        // Verify that visiting the direct connections route redirects to billing (302)
        $responseConnection = $this->get($this->storeUrl($tenant, 'woo/connections'));
        $responseConnection->assertStatus(302);
    ...
    public function test_new_features_plan_gating(): void
    {
        ...
        $tenantStarter = $this->createTenant('starter-gated-store', 'ltd_1');
        $this->actingAsTenantUser($tenantStarter, 'owner');

        // Under V11 Universal Spec, production, e_invoicing, bank_reconciliation, marketing_campaigns are universal
        $this->get($this->storeUrl($tenantStarter, 'inventory/production'))->assertStatus(200);
        $this->get($this->storeUrl($tenantStarter, 'e-invoicing'))->assertStatus(200);
        $this->get($this->storeUrl($tenantStarter, 'bank-reconciliation'))->assertStatus(200);
        $this->get($this->storeUrl($tenantStarter, 'marketing/campaigns'))->assertStatus(200);

        // 2. Create a Growth plan tenant (ltd_2 maps to growth)
        $tenantGrowth = $this->createTenant('growth-gated-store', 'ltd_2');
        $this->actingAsTenantUser($tenantGrowth, 'owner');

        // Growth should also successfully access (200 status)
