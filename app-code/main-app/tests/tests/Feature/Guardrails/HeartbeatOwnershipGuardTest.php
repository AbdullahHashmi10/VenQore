...
class HeartbeatOwnershipGuardTest extends VenQoreTestCase
{
    ...
    public function test_legitimate_heartbeat_from_owning_store_returns_alive(): void
    {
        ...
        // Recheck SEC-04 (2026-09-10): a paired terminal proves itself with its
        // device secret — the device ID alone no longer earns a response.
        $secret = \App\Support\TerminalDeviceAuth::issue($terminal);
        app()->forgetInstance('current.tenant');

        $response = $this->withHeaders(['X-Device-Secret' => $secret])->postJson('/api/heartbeat', [
            'device_id'  => 'owner-hb-device-999',
            'store_slug' => $store->slug,
            'status'     => 'OPEN',
        ]);
