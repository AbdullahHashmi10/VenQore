...
class SaleFinancialValueGuardTest extends VenQoreTestCase
{
    ...
    {
        $tenant = $this->createTenant(null, 'scale');
    ...
    public function test_sale_reversal_restores_financials_and_stocks(): void
    {
        ...
        $reversalSummary = DB::transaction(fn () => app(\App\Services\V3\SaleReversalService::class)->reverse(
            $saleModel,
            'returned',
    ...
    public function test_purchase_stores_exact_money_values(): void
    {
        ...
        $purchase = app(\App\Services\V3\PurchaseService::class)->createPurchase([
        ...
        $row = DB::table('purchases')->where('id', $purchase->id)->first();
        $this->assertNotNull($row, 'Purchase bill was not written.');

        $this->assertMoneyEquals(400.00, (float) $row->subtotal, 'subtotal wrong');
        $this->assertMoneyEquals(40.00,  (float) $row->discount, 'discount wrong');
        $this->assertMoneyEquals(20.00,  (float) $row->tax,      'tax wrong');
        $this->assertMoneyEquals(380.00, (float) $row->total,    'total wrong');

        // Supplier balance should be updated (since it is unpaid, we owe them 380 on Account 2000)
        $payableBalance = app(\App\Services\V3\PartyService::class)->getBalance($supplier->id, '2000');
        $this->assertMoneyEquals(380.00, $payableBalance, 'supplier payable balance wrong');
