...
class OfflineSyncIdempotencyGuardTest extends VenQoreTestCase
{
    ...
    public function test_offline_sync_idempotency_under_missing_tenant_context(): void
    {
        ...
        // Should not duplicate the sale. Since SEC-02 (2026-09-10) the store is
        // resolved ONLY by the store.member middleware (active membership
        // proven); with no bound store context the controller fails CLOSED
        // (403) instead of trusting users.last_store_id.
        $this->assertSame(
            1,
            DB::table('sales')->where('id', $clientId)->count(),
