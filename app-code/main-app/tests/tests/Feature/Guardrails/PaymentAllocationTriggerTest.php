 ...
 *
 * MySQL/MariaDB-only (the trigger is MySQL-dialect SQL, which MariaDB runs unchanged;
 * CLAUDE.md forbids SQLite). Laravel reports MariaDB as its own `mariadb` driver, which is
 * what production and this suite use. The payment_allocations table was renamed to
 * `allocations` (2026_08_12_084955_unify_allocations_tables), which also re-created the
 * triggers on the new table.
 */
class PaymentAllocationTriggerTest extends VenQoreTestCase implements \Tests\Support\RequiresGoldenCompany
{
    private function requireMysqlWithTrigger(): void
    {
        if (! in_array(DB::connection()->getDriverName(), ['mysql', 'mariadb'], true)) {
            $this->markTestSkipped('PaymentAllocation trigger is MySQL/MariaDB-only.');
        }
        // Scope to THIS database: other schemas on the same server carry their own copy.
        $exists = DB::select(
            "SELECT COUNT(*) c FROM information_schema.triggers
              WHERE trigger_schema = DATABASE()
                AND trigger_name = 'chk_allocation_insert'
                AND event_object_table = 'allocations'"
        );
        if (($exists[0]->c ?? 0) < 1) {
    ...
    public function trigger_rejects_over_allocation_of_a_payment(): void
    {
        ...
        // A valid allocation (80 <= 100) must succeed. (allocations.id is a UUID — no auto-increment.)
        $okId = \Illuminate\Support\Str::uuid()->toString();
        ...
        $this->assertTrue(
            DB::table('allocations')->where('id', $okId)->exists(),
            'Valid allocation within the payment total should be accepted.'
        );
