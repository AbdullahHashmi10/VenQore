...
class LaunchGateTest extends VenQoreTestCase implements RequiresGoldenCompany
{
    ...
    public function test_G03_number_registry_has_zero_critical_issues(): void
    {
        ...
        $qPath = file_exists(base_path('tests/VerificationCenter/registry/quarantine.yaml'))
            ? base_path('tests/VerificationCenter/registry/quarantine.yaml')
            : base_path('Tester/VerificationCenter/registry/quarantine.yaml');
        if (is_file($qPath)) {
    ...
    {
        $path = __DIR__ . '/ArchitecturalEnforcementTest.php';
