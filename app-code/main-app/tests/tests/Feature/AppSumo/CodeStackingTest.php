...
class CodeStackingTest extends VenQoreTestCase
{
    ...
    public function third_code_upgrades_to_ltd_3_with_correct_unlimited_limits(): void
    {
        ...
        $this->assertNull($tenant->getLimit('transactions_per_month'),
            'ltd_3 transactions must be unlimited (null per V11 spec Part 7).');
        $this->assertEquals(5, $tenant->getLimit('staff_limit'),
            'ltd_3 staff_limit is 5 full seats per V11 spec.');
        $this->assertEquals(50000, $tenant->getLimit('sku_limit'),
            'ltd_3 sku_limit is 50000 per V11 spec.');
    }
    ...
    public function invalid_code_returns_422(): void
    {
        ...
        $err = $response->json();
        fwrite(STDERR, "\n--- ERROR: " . ($err['message'] ?? 'no msg') . ' | ' . ($err['exception'] ?? 'no exc') . "\n");
    ...
    public function transaction_limit_blocks_write_at_threshold(): void
    {
        ...
        // V7 Spec §7.1: transactions_per_month is null (unlimited) on all LTD tiers
        $this->assertNull($tenant->getLimit('transactions_per_month'));
    }
    ...
    public function read_access_is_never_blocked_at_limit(): void
    {
        ...
        $this->assertNotEquals(402, $response->status(),
            'Read access must never be blocked.');
