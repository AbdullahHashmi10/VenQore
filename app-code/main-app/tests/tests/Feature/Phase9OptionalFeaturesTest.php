...
class Phase9OptionalFeaturesTest extends TestCase
{
    ...
    {
        // In V11, bulk_upload is universal (ON for all plans including solo/starter/core/scale)
        $starterTenant = Tenant::factory()->create(['plan' => 'starter']);
        app()->instance('current.tenant', $starterTenant);

        $this->assertTrue(PlanGate::check('bulk_upload'), 'Starter plan should have bulk_upload entitlement in V11.');

        $growthTenant = Tenant::factory()->create(['plan' => 'growth']);
        app()->instance('current.tenant', $growthTenant);

        $this->assertTrue(PlanGate::check('bulk_upload'), 'Growth/Core plan must have bulk_upload entitlement.');
    }
