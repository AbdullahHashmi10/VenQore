...
class ServiceOnlySaleTest extends VenQoreTestCase
{
    ...
    public function the_ledger_balances_after_a_service_sale(): void
    {
        ...
        $saleRef = $sale->reference_number ?? $sale->invoice_number ?? $sale->id;
        $entry = DB::table('journal_entries')
            ->where('reference', $sale->id)
            ->orWhere('reference', $saleRef)
    ...
    public function the_invoice_number_comes_from_the_sequence_service(): void
    {
        ...
        foreach ([$first, $second] as $sale) {
            $refNum = $sale->reference_number ?? $sale->invoice_number;
        ...
        $this->assertNotSame($first->reference_number ?? $first->invoice_number, $second->reference_number ?? $second->invoice_number);
    }
    ...
    public function a_part_fitted_during_a_job_does_move_stock(): void
    {
        ...
        $batchBefore = DB::table('inventory_batches')->where('product_id', $product->id)->value('remaining_qty');
        $this->billing->invoiceJob($job->fresh());
        $batchAfter = DB::table('inventory_batches')->where('product_id', $product->id)->value('remaining_qty');

        $this->assertEquals(
            $batchBefore - 1,
            $batchAfter,
            'A physical part on a service job must still deduct stock from inventory batches. Only labour is exempt.'
        );
        $this->assertGreaterThan(
            0,
            DB::table('sale_item_batches')->count(),
            'Sale item batches audit record must be created for physical parts on a service job.'
        );
    }
    ...
    private function receiveStock($product, int $qty, float $cost): void
    {
        $warehouseId = DB::table('warehouses')->where('tenant_id', $this->tenant->id)->value('id');
        if (!$warehouseId) {
