...
$standaloneDirectories = ['Smoke', 'DemoStore', 'Golden', 'Module07', 'Chat', 'Billing', 'Module19', 'Monetization', 'Money'];
...
foreach (glob(__DIR__ . '/Feature/*', GLOB_ONLYDIR) as $dir) {
    ...
    pest()->extend(VenQoreTestCase::class)->in("tests\\Feature\\{$dirName}");
    pest()->extend(VenQoreTestCase::class)->in("tests/tests/Feature/{$dirName}");
    pest()->extend(VenQoreTestCase::class)->in("tests\\tests\\Feature\\{$dirName}");
}

// Root-level Feature test files (ProfileTest.php, ImportMappingTest.php, etc.)
foreach (glob(__DIR__ . '/Feature/*.php') as $file) {
    if (in_array(basename($file), ['VenQoreTestCase.php', 'LedgerAccountingIntegrationTest.php'], true)) {
        continue; // base class or declares uses() itself
    }
    ...
    pest()->extend(VenQoreTestCase::class)->in("tests\\Feature\\{$fileName}");
    pest()->extend(VenQoreTestCase::class)->in("tests/tests/Feature/{$fileName}");
    pest()->extend(VenQoreTestCase::class)->in("tests\\tests\\Feature\\{$fileName}");
}
