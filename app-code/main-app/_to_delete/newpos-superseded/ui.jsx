...
export function Icon({ children, label, onClick, on, rank, title, className = '' }) {
    return (
        <button
            type="button"
            className={`nqp-iconbtn ${className}`}
            aria-label={label}
            title={title || label}
...
export function Flag({ tone, children, title }) {
    return <span className="nqp-flag" data-tone={tone} title={title}>{children}</span>;
}

export function Kbd({ children }) { return <kbd className="nqp-kbd">{children}</kbd>; }
...
export function Sheet({ open, onClose, title, subtitle, size = 'side', side, children, footer, labelExtra }) {
    ...
    return (
        ...
        >
            <header className="nqp-sh">
                ...
                {labelExtra}
                <span style={{ flex: 1 }} />
                <Icon label="Close" onClick={onClose}>✕</Icon>
            </header>
            <div className="nqp-pb">{children}</div>
            {footer ? <div className="nqp-sf">{footer}</div> : null}
        </aside>
    );
}

/* ── Settings controls ───────────────────────────────────────────────────── */
export function Seg({ label, value, options, labels, onPick, note, disabled }) {
    return (
        <div className="nqp-ctl">
            {label ? <div className="lbl"><span>{label}</span></div> : null}
            <div className="nqp-seg" role="group" aria-label={label}>
                {options.map((o, i) => (
                    <button
                        key={String(o)}
                        type="button"
...
export function Slider({ label, value, lo, hi, step, fmt, onSet, note }) {
    return (
        <div className="nqp-ctl">
...
export function Switch({ label, note, value, onChange }) {
    return (
        <button type="button" className="nqp-switch" aria-pressed={!!value} onClick={() => onChange(!value)}>
...
export function Toasts({ items, onAction, onDismiss }) {
    return (
        <div className="nqp-toasts" aria-live="polite">
            {items.map((t) => (
                <div className="nqp-toast" key={t.id} data-tone={t.tone}>
