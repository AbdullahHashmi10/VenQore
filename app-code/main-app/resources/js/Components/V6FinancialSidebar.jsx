...
export default function V6FinancialSidebar({
  ...
  return (
    <div className={`w-full h-full flex flex-col gap-2 text-white justify-between ${className}`}>
      <CashDetailModal
      ...
      <div className="flex flex-col gap-0.5 px-1 pt-0 shrink-0">
        <div className="flex items-center gap-1.5">
          <div className="w-6 h-6 rounded-lg bg-teal-500/10 dark:bg-white/[0.06] border border-teal-500/20 dark:border-white/[0.10] flex items-center justify-center text-teal-600 dark:text-white shadow-inner shrink-0">
            <Wallet size={13} strokeWidth={2.4} />
          </div>
          <p className="text-[9.5px] font-bold text-slate-500 dark:text-neutral-400 uppercase tracking-widest leading-none">
            TOTAL BALANCE
          </p>
        </div>
        <div className="flex items-baseline justify-end">
          <span 
            className="text-2xl sm:text-[26px] font-extrabold tracking-tight text-slate-900 dark:text-white leading-none whitespace-nowrap"
            style={{ 
      ...
      <div className="relative shrink-0" ref={menuRef}>
        <div className="grid grid-cols-3 gap-1.5">
          ...
          <button
            ...
            className="bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/25 hover:border-emerald-500/40 text-emerald-700 dark:bg-emerald-500/[0.10] dark:hover:bg-emerald-500/[0.20] dark:border-emerald-500/30 dark:hover:border-emerald-500/50 dark:text-emerald-400 rounded-xl py-1.5 px-1 flex flex-col items-center justify-center gap-0.5 transition-all duration-200 active:scale-95 group shadow-sm backdrop-blur-sm"
          >
            <div className="w-6 h-6 rounded-lg bg-emerald-500/20 group-hover:bg-emerald-500 group-hover:text-white dark:group-hover:text-black flex items-center justify-center transition-all duration-200">
              <ArrowDownLeft size={13} strokeWidth={2.5} />
            </div>
            <span className="text-[9.5px] font-black tracking-wider">SALE</span>
          </button>
          ...
          <button
            ...
            className="bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/25 hover:border-amber-500/40 text-amber-700 dark:bg-amber-500/[0.10] dark:hover:bg-amber-500/[0.20] dark:border-amber-500/30 dark:hover:border-amber-500/50 dark:text-amber-400 rounded-xl py-1.5 px-1 flex flex-col items-center justify-center gap-0.5 transition-all duration-200 active:scale-95 group shadow-sm backdrop-blur-sm"
          >
            <div className="w-6 h-6 rounded-lg bg-amber-500/20 group-hover:bg-amber-500 group-hover:text-white dark:group-hover:text-black flex items-center justify-center transition-all duration-200">
              <ArrowUpRight size={13} strokeWidth={2.5} />
            </div>
            <span className="text-[9.5px] font-black tracking-wider">PURCHASE</span>
          </button>
          ...
          <button
            ...
            className={`bg-teal-500/10 hover:bg-teal-500/20 border border-teal-500/25 hover:border-teal-500/40 text-teal-700 dark:bg-teal-500/[0.10] dark:hover:bg-teal-500/[0.20] dark:border-teal-500/30 dark:hover:border-teal-500/50 dark:text-teal-300 rounded-xl py-1.5 px-1 flex flex-col items-center justify-center gap-0.5 transition-all duration-200 active:scale-95 group shadow-sm backdrop-blur-sm ${isMenuOpen ? 'ring-2 ring-teal-500/50 bg-teal-500/25' : ''}`}
          >
            <div className="w-6 h-6 rounded-lg bg-teal-500/20 group-hover:bg-teal-500 dark:group-hover:bg-teal-400 group-hover:text-white dark:group-hover:text-black flex items-center justify-center transition-all duration-200">
              <Plus size={13} strokeWidth={2.5} />
            </div>
            <span className="text-[9.5px] font-black tracking-wider">ACTIONS</span>
          </button>
        </div>
      ...
      {/* 3. Cash in Hand Card (rounded-[16px], Label on left, Number on right) */}
      <button 
        type="button"
        aria-label="View Cash in Hand Details"
        onClick={() => setIsCashModalOpen(true)}
        className="w-full text-left bg-black/[0.03] hover:bg-black/[0.06] dark:bg-white/[0.04] dark:hover:bg-white/[0.07] border border-black/[0.06] hover:border-black/[0.12] dark:border-white/[0.08] dark:hover:border-white/[0.16] rounded-[16px] p-2.5 flex items-center justify-between transition-all duration-200 cursor-pointer shadow-sm relative overflow-hidden group shrink-0"
      >
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-lg bg-emerald-500/15 border border-emerald-500/25 flex items-center justify-center text-emerald-600 dark:text-emerald-400 shrink-0">
            <Wallet size={13} strokeWidth={2.2} />
          </div>
          <span className="text-xs font-bold text-slate-800 dark:text-neutral-200">Cash in Hand</span>
        </div>
        <span 
          className="text-base sm:text-[17px] font-extrabold tracking-tight text-slate-900 dark:text-white text-right whitespace-nowrap"
      ...
      {/* 4. Stock Value Card (rounded-[16px], Label on left, Number on right) */}
      <button 
        type="button"
      ...
      >
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-lg bg-teal-500/15 border border-teal-500/25 flex items-center justify-center text-teal-600 dark:text-teal-400 shrink-0">
      ...
      <div className="shrink-0 flex flex-col">
        <div className="flex items-center justify-between px-1 mb-1">
          <p className="text-[9.5px] font-bold text-slate-500 dark:text-neutral-400 uppercase tracking-widest">
            BANK ACCOUNTS
          </p>
          <button
            type="button"
            onClick={() => handleNavigate('store.bank-accounts.index', { action: 'add' })}
            className="flex items-center gap-1 text-[9.5px] font-bold text-teal-600 dark:text-teal-400 hover:text-teal-700 dark:hover:text-teal-300 bg-teal-500/10 hover:bg-teal-500/20 border border-teal-500/25 px-2 py-0.5 rounded-full transition-all"
          >
        ...
        <div className="flex flex-col gap-1.5">
          {displayBankAccounts.map((acc) => (
            <button
              ...
              className="w-full text-left bg-black/[0.03] hover:bg-black/[0.06] dark:bg-white/[0.04] dark:hover:bg-white/[0.08] border border-black/[0.06] hover:border-black/[0.12] dark:border-white/[0.08] dark:hover:border-white/[0.16] rounded-[16px] p-2 flex flex-col gap-0.5 transition-all duration-200 cursor-pointer group shadow-sm shrink-0"
            >
              {/* Line 1: Bank Name on left, Account last digits on right */}
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <div className="w-5 h-5 rounded-md bg-teal-500/10 border border-teal-500/20 text-teal-600 dark:text-teal-400 flex items-center justify-center group-hover:scale-105 transition-transform shrink-0">
                    <Building2 size={11} strokeWidth={2} />
                  </div>
                  <p className="text-[11.5px] font-bold text-slate-800 group-hover:text-slate-950 dark:text-neutral-100 dark:group-hover:text-white transition-colors leading-tight">
                    {acc.bank_name || acc.name}
                  </p>
                </div>
                <span 
                  className="text-[9.5px] text-slate-500 dark:text-neutral-400 font-medium whitespace-nowrap"
              ...
              <div className="flex items-center justify-end">
                <span 
                  className={`text-[16px] sm:text-[17px] font-extrabold tracking-tight whitespace-nowrap leading-tight ${parseFloat(acc.current_balance || 0) < 0 ? 'text-rose-500 dark:text-rose-400' : 'text-slate-900 dark:text-white'}`}
                  style={{ fontFamily: 'var(--vq-font-numeric)', fontVariantNumeric: 'tabular-nums', fontWeight: 800, letterSpacing: '-0.02em' }}
                >
      ...
      {/* 6. Activity Card (compact, rounded-[16px], flex-1 min-h-0 to adapt dynamically) */}
      <div className="bg-black/[0.02] dark:bg-white/[0.03] border border-black/[0.06] dark:border-white/[0.08] rounded-[16px] p-2 shadow-sm flex flex-col flex-1 min-h-[90px] overflow-hidden">
        {/* Header with Legend */}
        <div className="flex justify-between items-center mb-1.5 px-0.5 shrink-0">
          <h3 className="font-bold text-[9.5px] text-slate-500 dark:text-neutral-300 uppercase tracking-widest">
            ACTIVITY
          </h3>
          <div className="flex items-center gap-2 text-[9px] font-semibold text-slate-500 dark:text-neutral-400">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar space-y-1 pr-0.5">
          {displayTransactions.map((tx, i) => {
            ...
            return (
              <div
                key={i}
                className="flex items-center justify-between px-1.5 py-1 rounded-lg bg-black/[0.02] hover:bg-black/[0.05] dark:bg-white/[0.02] dark:hover:bg-white/[0.05] border border-black/[0.03] dark:border-transparent transition-colors cursor-pointer group"
              >
                <div className="flex items-center gap-1.5">
                  <div className={`w-4.5 h-4.5 rounded-md flex items-center justify-center text-[9px] font-bold shrink-0 ${isSale ? 'bg-teal-500/15 text-teal-600 dark:bg-teal-500/20 dark:text-teal-300 border border-teal-500/25 dark:border-teal-500/30' : 'bg-amber-500/15 text-amber-600 dark:bg-amber-500/20 dark:text-amber-300 border border-amber-500/25 dark:border-amber-500/30'}`}>
                    {isIncoming ? <ArrowDownLeft size={10} strokeWidth={2.4} /> : <ArrowUpRight size={10} strokeWidth={2.4} />}
                  </div>
                  <div>
                    <div className="flex items-center gap-1">
                      <span className={`w-1 h-1 rounded-full ${isSale ? 'bg-teal-500 dark:bg-teal-400' : 'bg-amber-500 dark:bg-amber-400'}`}></span>
                      <span className="text-[10px] font-bold text-slate-800 group-hover:text-slate-950 dark:text-neutral-200 dark:group-hover:text-white transition-colors leading-tight">
                        {tx.type || 'Transaction'}
                      </span>
                    </div>
                    <span className="text-[8.5px] text-slate-500 dark:text-neutral-400 font-medium block pl-2 leading-tight">
                      {tx.time || 'Recently'}
                    </span>
                  </div>
                </div>
                <span 
                  className={`text-[11.5px] sm:text-xs font-extrabold tracking-tight whitespace-nowrap ${isIncoming ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'}`}
