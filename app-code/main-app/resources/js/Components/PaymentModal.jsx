...
function PartySearchField({ selectedParty, onSelect, onClear, store, isIn }) {
    ...
    return (
        <div ref={containerRef} className="relative">
            <div style={{ outline: 'none' }} className={`flex items-center gap-2 w-full px-3 py-3 rounded-xl bg-app border ${selectedParty ? (isIn ? 'border-emerald-500' : 'border-rose-500') : 'border-line'} transition-all focus-within:border-brand-500`}>
                ...
                />
                {loading && <div className={`w-4 h-4 border-2 border-line ${isIn ? 'border-t-emerald-500' : 'border-t-rose-500'} rounded-full animate-spin shrink-0`} />}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-ink-muted text-sm font-bold">{getCurrencySymbol(store)}</span>
                            <input
                                type="number"
