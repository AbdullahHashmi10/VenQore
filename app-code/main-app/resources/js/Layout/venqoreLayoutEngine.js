...
export function composeTerminal(comp, vw, vh) {
  ...
  /* THE WATERFALL, as §10 states it: "every resident pane reserves its leanest
     floor, in rank order; panes are upgraded one fit-step at a time; whatever
     is left is shared by weight, respecting caps." The document composer above
     already does this (see the paneCaps/absorbers pass around line 3140); the
     terminal did not, and two things were missing.

     1. THE FLOOR WAS WEIGHTED LIKE A COLUMN, at a hard-coded .20. On a 1024
        iPad that works out below the floor pane's own measured 254px minimum,
        so the pane §10 gives a resident column at every width from 1024 up was
        demoting itself to a step at all of them. The floor is `hold:
        residency` -- presence is the information -- so it reserves its leanest
        fit BEFORE anything is shared out, and grows only once the fixed panes
        are full.

     2. THE CAPS WERE NEVER APPLIED to the terminal. The law's central claim is
        that a wider screen shows MORE, not bigger: a cart shows the same five
        columns however wide it is, which is why it is capped at 805 and the
        tender at 551. Unapplied, a 3440 ultrawide handed the cart 1629px of
        margin while the catalog stayed three tiles across. The surplus now
        goes where the law says it goes -- to the absorbers, which are the
        panes that genuinely show more when given room. */
  ...
  const allocateColumns = (wantCat, wantFloor, wantTender) => {
    ...
    if (surplus > 0) {
      const abs = Object.keys(px).filter(k => LAW.absorbers.includes(k));
      if (abs.length) {
        const aw = abs.reduce((a, k) => a + px[k], 0) || 1;
        for (const k of abs) px[k] += surplus * px[k] / aw;
      } else {
        /* Nothing on this screen absorbs. Handing it back to the cart is not
           "bigger is better" -- it is that a track short of the pool leaves a
           visible gap, and a gap is worse than a wide cart. */
        px.cart += surplus;
      }
    }
      ...
      const wantTender2 = px.tender !== undefined;
      if (wantTender2) {
  ...
  /* WHEN THE FLOOR EARNS A COLUMN.

     The old test was `avail >= RESIDENT_MIN + CAT_LIST + G` -- room for a
     CATALOG column plus a list. But the table terminal's catalog is a BAND
     (§10: "1 row x 6 tiles" from 1024 up), so this was refusing the floor a
     column on the grounds that a pane which is not a column would not fit
     beside it. The result: §10 gives the floor a resident column at every
     width from 1024, and the register was showing a step until 1521.

     The honest test is what actually has to stand side by side -- the floor at
     its leanest fit, the cart at its own floor, and the tender and catalog
     only when THEY are columns too. */
  ...
  if (catMode === "off") cat = null;
    ...
    if (!rows) {
      ...
      if (!dock.some(d => d.id === "catalog")) {
        ...
        /* No row for it -- see the amendment above. It goes in the cart header. */
      }
    } else {
