...
function PartySearchField({ selectedParty, onSelect, onClear }) {
    ...
    const balanceBadge = () => {
        ...
        if (Math.abs(bal) < 0.01) return (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-sunken text-ink-muted text-2xs font-bold">
    ...
    return (
        <div ref={containerRef} className="relative">
            <div style={{ outline: 'none' }} className={`flex items-center gap-2 w-full px-3 py-3 rounded-xl bg-app border ${selectedParty ? 'border-rose-500' : 'border-line'} transition-all focus-within:border-rose-500`}>
                ...
                />
                {loading && <div className="w-4 h-4 border-2 border-line border-t-rose-500 rounded-full animate-spin shrink-0" />}
                {(query || selectedParty) && !loading && (
                    <button type="button" onClick={handleClear} className="text-ink-muted hover:text-ink-secondary dark:hover:text-neutral-200 transition shrink-0">
                        <X size={14} />
                    </button>
                )}
            </div>
            ...
            {selectedParty && (
                <div className="mt-1.5 flex items-center gap-2 px-1">
                    <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs font-semibold text-ink-secondary">{selectedParty.name}</span>
                        {selectedParty.type && (
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-semibold text-ink truncate">{party.name}</p>
                                ...
                                {settled && <span className="text-2xs font-bold px-1.5 py-0.5 rounded-full shrink-0 bg-sunken text-ink-muted dark:bg-raised dark:text-ink-muted">Settled</span>}
                            </button>
                        );
                    })}
                </div>
        ...
        <OneGlanceLayout title="Payment Out">
            ...
            <div className="h-full flex flex-col items-center justify-center overflow-auto py-6 px-4">
                ...
                <div className="w-full max-w-xl relative">
                    ...
                    {/* Glow */}
                    <div className="absolute -inset-1 bg-gradient-to-r from-rose-500/20 to-red-500/10 rounded-2xl blur-xl pointer-events-none" />
                    ...
                    <div className="relative bg-surface rounded-2xl border border-line shadow-2xl overflow-hidden">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-5">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div className="relative">
                                        <CalendarDays size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-ink-muted" />
                                        <input
                                            type="date"
                                            value={formData.date}
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-ink-muted text-sm font-bold">{getCurrencySymbol()}</span>
                                        <input
                                            type="number"
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`flex flex-col items-center gap-1.5 py-2.5 px-2 rounded-xl border-2 transition-all text-2xs font-bold uppercase ${isSelected
                                                    ...
                                                    : 'border-line text-ink-muted hover:border-line dark:hover:border-line-strong bg-app'
                                                    }`}
                                            >
                                                <m.icon size={16} />
                                                {m.label}
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-2xs font-bold uppercase tracking-wider text-ink-muted mb-1.5">Reference No.</label>
                                    <div className="relative">
                                        <Hash size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-ink-muted" />
                                        <input
                                            type="text"
                                ...
                                <div>
                                    <label className="block text-2xs font-bold uppercase tracking-wider text-ink-muted mb-1.5">Description</label>
                                    <div className="relative">
                                        <FileText size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-ink-muted" />
                                        <input
                                            type="text"
