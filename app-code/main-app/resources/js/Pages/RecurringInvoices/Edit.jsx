...
const EditRecurringInvoice = ({ invoice, customers = [], warehouses: initialWarehouses = [], products = [] }) => {
    ...
    useEffect(() => {
        if (invoice) {
            ...
            const mappedItems = (invoice.items || []).map((item, idx) => {
                // Find matching product if products prop is available
                const matchedProduct = products.find(p => p.id === item.product_id);
                return {
                    id: item.id || Date.now() + idx,
                    product: matchedProduct || { id: item.product_id, name: item.name || 'Product', price: item.price || item.unit_price || 0 },
                    name: item.name || matchedProduct?.name || 'Product',
                    quantity: parseFloat(item.quantity || item.qty) || 1,
                    originalQuantity: parseFloat(item.quantity || item.qty) || 0,
                    price: parseFloat(item.price || item.unit_price) || 0,
                    cost: parseFloat(matchedProduct?.cost || matchedProduct?.cost_price || item.cost || 0),
                    discount: parseFloat(item.discount || item.discount_percent || 0),
                    discountType: item.discountType || item.discount_type || 'fixed',
                    freeQuantity: parseFloat(item.freeQuantity || item.free_qty || 0)
                };
            });

            // Ensure there is at least one item row, or if the last item is populated we append an empty one like in Sales
            if (mappedItems.length === 0) {
                mappedItems.push({ id: Date.now(), product: null, quantity: 1, freeQuantity: 0, price: 0, discount: 0, discountType: 'fixed' });
            }
            ...
            setEditState({
                ...
                next_run_date: invoice.next_run_date ? new Date(invoice.next_run_date).toISOString().split('T')[0] : new Date(Date.now() + 86400000 * 30).toISOString().split('T')[0],
                status: invoice.status || 'active',
                paymentMethod: 'cash', // Fallback defaults
                amountPaid: 0,
                discount: 0,
                tax: 0,
                delivery_charge: 0,
    ...
    const activeInvoices = editState ? [editState] : [];
    const currentInvoiceId = editState?.id || 'edit-recurring';
    const setCurrentInvoiceId = () => {};
    const addInvoice = () => {};
    const removeInvoice = () => {};
    const updateInvoice = (id, data) => {
        setEditState(prev => prev && prev.id === id ? { ...prev, ...data } : prev);
    };

    const currentInvoice = editState || {
        items: [],
        customer: null,
        discount: 0,
        tax: 0,
    ...
    const patchInvoice = (data) => {
        if (editState) {
            updateInvoice(editState.id, data);
        }
    };
    ...
    const processSale = async () => {
        ...
        try {
            ...
            const response = await axios.put(route('store.recurring-invoices.update', { store_slug: store?.slug, id: currentInvoice.id }), payload);

            if (response.data) {
                localStorage.setItem('amd_product_latest_change', Date.now().toString());
                showAlert({ title: 'Success', message: 'Recurring invoice template updated successfully.', type: 'success' });
            ...
            showAlert({
                title: 'System Error',
                message: error.response?.data?.message || 'Failed to update recurring invoice template.',
                type: 'error'
            });
        } finally {
            setSaving(false);
                    ...
                    <div className="hidden lg:flex w-full lg:w-80 bg-void-700 flex-col overflow-hidden rounded-2xl shadow-2xl border border-neutral-800 shrink-0">
                        ...
                        <div className="flex-1 p-3 space-y-3 overflow-y-auto hide-scrollbar">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    <label className="text-3xs text-ink-muted font-bold uppercase block mb-1">Warehouse</label>
                                    <select
                                ...
                                <div>
                                    <label className="text-3xs text-ink-muted font-bold uppercase block mb-1">Billing Frequency</label>
                                    <select
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    <label className="text-3xs text-ink-muted font-bold uppercase block mb-1">Next Run Date</label>
                                    <input
                                        type="date"
                                ...
                                <div>
                                    <label className="text-3xs text-ink-muted font-bold uppercase block mb-1">Status</label>
                                    <select
