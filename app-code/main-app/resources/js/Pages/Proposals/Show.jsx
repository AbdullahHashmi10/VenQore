                        ...
                        <div className="bg-surface rounded-2xl shadow-lg border border-line p-8 print-container">
                            ...
                            <div className="flex justify-between items-start mb-12">
                                ...
                                <div className="text-right">
                                    <h2 className="text-3xl font-bold text-ink mb-2">PROPOSAL</h2>
                                    <p className="text-ink-muted font-medium">#{proposal.reference_number}</p>
                                    <div className="mt-4 space-y-1 text-sm">
                                        <p className="text-ink-muted">Date: <span className="text-ink font-medium">{new Date(proposal.created_at).toLocaleDateString()}</span></p>
                                        {proposal.valid_until && (
                                            <p className="text-ink-muted">Valid Until: <span className="text-ink font-medium">{new Date(proposal.valid_until).toLocaleDateString()}</span></p>
                                        )}
                                        <p className="text-ink-muted">Status: <span className={`uppercase font-bold px-2 py-0.5 rounded-lg text-xs ${getStatusColor(proposal.status)}`}>{proposal.status}</span></p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="border-t border-b border-line py-8 mb-8 grid grid-cols-2 gap-8">
                                <div>
                                    <h3 className="text-xs font-bold text-ink-muted uppercase tracking-wider mb-3">Prepared For</h3>
                                    {proposal.customer ? (
                                        <div className="space-y-1">
                                            <p className="font-bold text-ink text-lg">{proposal.customer.name}</p>
                                        ...
                                        <p className="font-bold text-ink text-lg">{proposal.customer_name || 'No Customer'}</p>
                                    )}
                                </div>
                                <div className="text-right">
                                    <h3 className="text-xs font-bold text-ink-muted uppercase tracking-wider mb-3">Proposal Details</h3>
                                    <p className="text-sm text-ink-muted">Created By: <span className="font-medium text-ink">{proposal.user?.name || 'Unknown'}</span></p>
                                </div>
                            </div>
                                ...
                                <table className="w-full text-left border-collapse">
                                    ...
                                    <tbody className="divide-y divide-line">
                                        {(proposal.items || []).map((item, index) => (
                                            <tr key={index}>
                                                <td className="py-4">
                                                    <p className="font-bold text-ink">{item.product?.name || item.product_name || 'Unknown Item'}</p>
                                                </td>
                                                <td className="py-4 text-center text-ink-secondary">{item.quantity}</td>
                                                <td className="py-4 text-right text-ink-secondary">{getCurrencySymbol()} {parseFloat(item.unit_price || 0).toLocaleString()}</td>
                                                <td className="py-4 text-right font-medium text-ink">{getCurrencySymbol()} {parseFloat(item.total || 0).toLocaleString()}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            ...
                            {/* Notes */}
                            {proposal.notes && (
                                <div className="mt-8 p-4 bg-app rounded-xl">
                                    <h4 className="text-xs font-bold text-ink-muted uppercase tracking-wider mb-2">Notes</h4>
                                    <p className="text-sm text-ink-secondary">{proposal.notes}</p>
                                </div>
                            )}
