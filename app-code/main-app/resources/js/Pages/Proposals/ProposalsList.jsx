...
export default function ProposalsList({ proposals = [], filters = {}, stats = {} }) {
 ...
 <span className="text-brand-600 dark:text-brand-400">Proposal: {stats?.total_count || sortedData.length}</span>
 ...
 <p className="text-xs font-bold text-ink-muted uppercase">Total Proposals</p>
 </div>
 <p className="text-base font-bold text-ink">{stats?.total_count || sortedData.length}</p>
 </div>
 <div className="bg-surface px-3 py-2 rounded-xl border border-line shadow-sm flex items-center justify-between">
 <div className="flex items-center gap-2">
 ...
 <p className="text-xs font-bold text-ink-muted uppercase">Accepted</p>
 </div>
 <p className="text-base font-bold text-emerald-600">{stats?.accepted_count || 0}</p>
 </div>
 <div className="bg-surface px-3 py-2 rounded-xl border border-line shadow-sm flex items-center justify-between">
 <div className="flex items-center gap-2">
 ...
 <p className="text-base font-bold text-ink">{formatCurrency(stats?.total_value || 0, store)}</p>
 </div>
 </div>
 ...
 Proposals / <span className="text-brand-600">Quotations</span>
 </h1>
...
`}
 ...
 <p className="text-lg font-bold text-ink-secondary mb-1">No proposals found</p>
 <p className="text-sm text-ink-muted mb-4">Create your first proposal to get started</p>
 <Link
 href={route('store.proposals.create', { store_slug: store?.slug })}
...
`}
 ...
 case 'reference': return <span className="font-mono text-brand-600 dark:text-brand-400 font-semibold">{row.proposal_number || `PROP-${row.id}`}</span>;
 case 'party_name':
 return (
 <div>
 ...
 return (
 <span className={isExpired ? 'text-red-500' : 'text-ink-muted'}>
 ...
 return <span className={`px-2 py-1 rounded-md text-xs font-bold uppercase ${statusStyles[row.status] || 'bg-sunken text-ink-secondary'}`}>{row.status || 'pending'}</span>;
 case 'actions':
 return (
 <div className="flex items-center justify-end gap-2 relative" onClick={(e) => e.stopPropagation()}>
 <a href={route('store.proposals.print', { store_slug: store?.slug, proposal: row.id })} target="_blank" className="p-1.5 hover:bg-interactive-hover dark:hover:bg-interactive-hover rounded-lg text-ink-muted hover:text-brand-600 transition-colors">
 <Printer size={16} />
 </a>
 ...
 <p className="text-sm font-bold text-ink-secondary">No proposals found</p>
 </div>
 ) : (
 sortedData.map((row) => {
 const isExpired = row.valid_until && new Date(row.valid_until) < new Date();
 ...
 <div>
 <span className="font-mono text-xs text-brand-600 dark:text-brand-400 font-bold">{row.proposal_number || `PROP-${row.id}`}</span>
...
`}>
 ...
 <p className="text-sm font-bold text-ink mt-0.5">{formatCurrency(row.total, store)}</p>
 </div>
 </div>

 <div className="flex justify-between items-center text-xs">
 <div className={isExpired ? 'text-red-500 font-bold' : 'text-ink-muted'}>
 ...
 <div>
 <p className="text-xs font-bold text-ink-muted uppercase tracking-wider">Proposal Preview</p>
 <h3 className="text-xl font-bold text-brand-600">{quickViewItem.proposal_number || `PROP-${quickViewItem.id}`}</h3>
 </div>
 {(() => {
 const statusStyles = {
 pending: 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400',
 ...
 <div className="bg-app p-3 rounded-xl">
 <p className="text-2xs font-bold text-ink-muted uppercase mb-1">Date Created</p>
 <p className="font-bold text-ink text-sm">{formatDate(quickViewItem.created_at)}</p>
 </div>
 <div className="bg-app p-3 rounded-xl">
 <p className="text-2xs font-bold text-ink-muted uppercase mb-1">Valid Until</p>
 <p className={`font-bold text-sm ${quickViewItem.valid_until && new Date(quickViewItem.valid_until) < new Date() ? 'text-red-600' : 'text-ink'}`}>
 ...
 <p className="font-bold text-brand-600 text-lg">{formatCurrency(quickViewItem.total, store)}</p>
 </div>
 </div>
 ...
 <p className="font-bold text-ink-secondary">{formatCurrency(quickViewItem.subtotal || quickViewItem.total, store)}</p>
 </div>
 <div className="text-right border-l border-line pl-8">
 <p className="text-2xs text-brand-600 uppercase font-bold">Grand Total</p>
 <p className="font-bold text-lg text-brand-600">{formatCurrency(quickViewItem.total, store)}</p>
 </div>
 </div>
 </div>
 </div>
