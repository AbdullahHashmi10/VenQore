    ...
    return (
        <OneGlanceLayout title="Stock Operations" activeMenu="Stock">
            ...
            <div className="h-full flex flex-col">
                ...
                {/* Content Area */}
                <div className="flex-1 overflow-hidden bg-app relative">
    ...
    return (
        <div className="h-full overflow-y-auto p-6 space-y-6">
            <div className="flex justify-between items-center">
                <h2 className="text-lg font-bold text-ink">Warehouse Management</h2>
                {!isAddingWarehouse && (
            ...
            {isAddingWarehouse ? (
                <div className="bg-surface rounded-xl border border-line p-6 max-w-2xl">
                    <h3 className="text-md font-bold text-ink mb-4">
                        {editingWarehouse ? 'Edit Warehouse Details' : 'Add New Warehouse'}
                    </h3>
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label className="block text-sm font-bold text-ink-secondary mb-2">Warehouse Name</label>
                            <input
                                type="text"
                                value={data.name}
                        ...
                        <div>
                            <label className="block text-sm font-bold text-ink-secondary mb-2">Location / Address</label>
                            <input
                                type="text"
                                value={data.location}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-bold text-ink-secondary mb-2">Contact Person (Optional)</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-sm font-bold text-ink-secondary mb-2">Phone (Optional)</label>
                                <input
                                    type="text"
                                    value={data.phone}
                ...
                <div className="space-y-4">
                    {(!warehouses || warehouses.length === 0) ? (
                        <div className="text-center py-12 bg-app rounded-xl border-2 border-dashed border-line">
                            ...
                            <p className="text-ink-muted font-medium">No warehouses yet</p>
                            <p className="text-sm text-ink-muted mt-1">Click "Add Warehouse" to create your first warehouse</p>
                        </div>
                    ) : (
                        warehouses.map(warehouse => {
                            ...
                            return (
                                <div key={warehouse.id} className="bg-surface p-4 rounded-xl border border-line flex justify-between items-center group hover:border-brand-500/30 transition-colors">
                                    <div>
                                        <div className="flex items-center gap-3 mb-1">
                                            <h3 className="font-bold text-ink">{warehouse.name}</h3>
                                            {isInfoMissing ? (
    ...
    return (
        <div className="h-full overflow-y-auto p-6 space-y-6">
            <div className="max-w-2xl">
                <h2 className="text-lg font-bold text-ink mb-6">Transfer Stock Between Warehouses</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-bold text-ink-secondary mb-2">From Warehouse</label>
                        <PremiumSelect
                    ...
                    <div>
                        <label className="block text-sm font-bold text-ink-secondary mb-2">To Warehouse</label>
                        <PremiumSelect
                    ...
                    <div>
                        <label className="block text-sm font-bold text-ink-secondary mb-2">Quantity to Transfer</label>
                        <input
                            type="number"
                            value={data.quantity}
                    ...
                    <div>
                        <label className="block text-sm font-bold text-ink-secondary mb-2">Notes (Optional)</label>
                        <textarea
                            value={data.notes}
                            onChange={(e) => setData('notes', e.target.value)}
                            rows="3"
                            className="w-full px-4 py-3 rounded-xl bg-surface border border-line focus:ring-2 ring-brand-500/20 outline-none resize-none"
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-1 shrink-0">
                ...
                <div className="bg-surface px-3 py-2 rounded-xl border border-line shadow-sm flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <p className="text-xs font-bold text-ink-muted uppercase">Total Stock</p>
                    </div>
                ...
                <div className="bg-surface px-3 py-2 rounded-xl border border-line shadow-sm flex items-center justify-between">
                    ...
                    <p className="text-base font-bold text-rose-600">{outOfStock}</p>
                </div>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 bg-surface px-3 py-2 rounded-xl border border-line shadow-sm shrink-0">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    <h1 className="text-lg font-bold text-ink uppercase tracking-tight shrink-0">
                        Stock <span className="text-brand-600">Adjustment</span>
                    </h1>
            ...
            <div className="flex-1 flex gap-2 overflow-hidden">
                ...
                <div className="flex-1 flex flex-col">
                    <form onSubmit={handleSubmit} className="flex-1 flex flex-col">
                        <div className="bg-surface rounded-xl border border-line shadow-sm flex-1 flex flex-col">
                            {/* Form Header */}
                            <div className="px-4 py-2 border-b border-line bg-app rounded-t-xl">
                                <p className="text-xs font-bold text-ink-muted uppercase">Adjustment Form</p>
                            </div>
                ...
                <div className="w-64 flex-shrink-0">
                    <div className="bg-surface rounded-xl border border-line shadow-sm h-full flex flex-col">
                        <div className="px-3 py-2 border-b border-line bg-app rounded-t-xl">
                            <p className="text-xs font-bold text-ink-muted uppercase">Live Preview</p>
                        </div>
                        <div className="p-3 flex-1 flex flex-col">
                            {selectedProduct ? (
                                <>
                                    {/* Product Info */}
                                    <div className="p-2 bg-app rounded-lg mb-2">
                                        <p className="font-bold text-ink text-sm truncate">{selectedProduct.name}</p>
                                        <p className="text-2xs text-ink-muted font-mono">SKU: {selectedProduct.sku}</p>
                                    </div>
                                    ...
                                    <div className="flex-1 flex flex-col justify-center items-center">
                                        ...
                                        <div className="text-center mb-1">
                                            <p className="text-2xs text-ink-muted uppercase tracking-wider">Current</p>
                                            <p className="text-2xl font-bold text-ink">{currentStock}</p>
                                        </div>
                                        ...
                                        <div className={`text-center p-2 rounded-xl w-full ${data.adjustment_type === 'add'
                                            ...
                                            <p className="text-2xs text-ink-muted uppercase tracking-wider">After</p>
                                            <p className={`text-2xl font-bold ${data.adjustment_type === 'add'
                                    ...
                                    {data.reason && (
                                        <div className="mt-2 text-center">
                                            <span className="inline-block px-2 py-0.5 bg-brand-100 dark:bg-brand-900/30 text-brand-700 dark:text-brand-400 rounded-full text-2xs font-bold">
                                                {data.reason}
                                            </span>
                                        </div>
                                    )}
...
function StockTake({ products, warehouses, defaultWarehouse, hasMultipleWarehouses, reasons }) {
    ...
    const ProductTable = () => (
        <table className="w-full relative">
            <thead className="bg-app sticky top-0 z-10 shadow-sm">
                <tr className="text-xs font-bold text-ink-muted uppercase tracking-wider">
                    <th className="p-4 text-center w-12">
                        <button onClick={toggleSelectAll} className="flex items-center justify-center text-ink-muted hover:text-brand-600">
            ...
            <tbody className="divide-y divide-line bg-surface">
                {filteredProducts?.map((product, index) => {
                    ...
                    return (
                        <tr key={product.id} className={`hover:bg-interactive-hover dark:hover:bg-interactive-hover transition-colors ${isSelected ? 'bg-brand-50/50 dark:bg-brand-900/10' : ''}`}>
                            <td className="p-4 text-center">
                                <button onClick={() => toggleSelect(product.id)} className={`flex items-center justify-center ${isSelected ? 'text-brand-600' : 'text-neutral-300 hover:text-ink-muted'}`}>
                                    {isSelected ? <CheckSquare size={18} /> : <Square size={18} />}
                                </button>
                            </td>
                            <td className="p-4 text-center text-ink-muted font-medium">{index + 1}</td>
                            <td className="p-4 font-medium text-ink">{product.name}</td>
                            <td className="p-4 text-ink-secondary">{product.sku}</td>
                            ...
                            <td className="p-4 text-center">
                                <input
                                    ...
                                    min="0"
                                    className="w-24 px-3 py-2 rounded-lg bg-app border border-line dark:border-line text-center font-bold focus:ring-2 ring-brand-500/20 outline-none transition-all"
                                />
                            </td>
                            <td className={`p-4 text-center font-bold ${difference > 0 ? 'text-green-600' : difference < 0 ? 'text-red-600' : 'text-ink-muted'
                                }`}>
    ...
    return (
        <>
            ...
            {showDependencyWarning && (
                <div className="fixed inset-0 z-sticky flex items-center justify-center p-4 bg-neutral-900/50 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-surface rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden border border-amber-200 dark:border-amber-900">
                        ...
                        <div className="p-6 max-h-[60vh] overflow-y-auto">
                            ...
                            <ul className="space-y-2 mb-6">
                                {dependencyData.map(item => (
                                    <li key={item.id} className="flex justify-between items-center p-3 bg-app rounded-lg border border-line">
                        ...
                        <div className="p-6 bg-app border-t border-line flex justify-end gap-3">
                            <button
                            ...
                            <button
                                ...
                                className="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-lg shadow-lg transition-all"
                            >
                ...
                <div className="fixed inset-0 z-sticky flex items-center justify-center p-4 bg-neutral-900/50 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-surface rounded-2xl shadow-2xl max-w-md w-full overflow-hidden">
                        <div className="p-6 text-center">
                            ...
                            <h3 className="text-xl font-bold text-ink mb-2">Move to Recycle Bin?</h3>
                        ...
                        <div className="p-6 bg-app border-t border-line flex justify-center gap-3">
                            <button
                                onClick={() => setShowDeleteConfirmation(false)}
                                className="px-6 py-3 text-ink-secondary font-bold hover:bg-interactive-hover dark:hover:bg-interactive-hover rounded-xl transition-colors"
                            >
                                Cancel
                            </button>
                            <button
            ...
            <div className={`h-full overflow-y-auto p-6 space-y-6 transition-all duration-slow ${isFullScreen ? 'fixed inset-0 z-50 bg-app p-8' : ''}`}>
                <div className={isFullScreen ? 'max-w-7xl mx-auto h-full flex flex-col' : ''}>
                    ...
                    {isFullScreen ? (
                        ...
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-lg font-bold text-ink">Physical Inventory Count (Bulk Update)</h2>
                            <div className="flex gap-3">
                                {selectedIds.size > 0 && (
                                    <div className="flex items-center gap-2 bg-sunken px-3 py-1 rounded-lg border border-line">
                                        <span className="text-xs font-bold text-ink-muted">{selectedIds.size} selected</span>
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-2 px-3 py-2 text-sm font-bold text-ink-secondary bg-surface border border-line rounded-lg hover:bg-interactive-hover dark:hover:bg-interactive-hover transition-colors"
                                >
                                    <Maximize2 size={18} />
                                    Focus Mode
                                </button>
                    ...
                    {isFullScreen && (
                        <div className="fixed inset-0 z-50 bg-sunken flex animate-in fade-in duration-slow">
                            ...
                            <div className="w-80 bg-surface border-r border-line flex flex-col shadow-xl z-20">
                                <div className="p-8">
                                    ...
                                    <div className="space-y-6">
                                        <div>
                                            ...
                                            <div className="relative">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full pl-12 pr-4 py-4 rounded-xl bg-app border border-line focus:ring-2 ring-brand-500/20 outline-none font-medium"
                                                    autoFocus
                                                />
                                            </div>
                                        </div>
                                        ...
                                        {hasMultipleWarehouses && (
                                            <div>
                                                <label className="block text-xs font-bold text-ink-muted uppercase tracking-wider mb-2">Current Warehouse</label>
                                                <div className="p-4 rounded-xl bg-app border border-line font-bold text-ink-secondary">
                            ...
                            <div className="w-24 bg-surface border-l border-line flex flex-col items-center py-8 gap-6 shadow-xl z-20">
                                <button
                                    onClick={() => setIsFullScreen(false)}
                                    className="p-4 rounded-xl text-ink-muted hover:bg-interactive-hover dark:hover:bg-interactive-hover hover:text-ink dark:hover:text-white transition-colors group"
                                    title="Exit Focus Mode"
                                >
                                    <Minimize2 size={24} className="transition-transform" />
                                </button>
                                    ...
                                    {selectedIds.size > 0 && (
                                        <>
                                            <div className="w-full px-2">
                                                <div className="h-px bg-sunken w-full" />
                                            </div>
                                            <span className="text-2xs font-bold text-ink-muted uppercase tracking-wider text-center">
                    ...
                    {!isFullScreen && hasMultipleWarehouses && (
                        <div className="mb-6">
                            <label className="block text-sm font-bold text-ink-secondary mb-2">Select Warehouse</label>
                            <select
                                value={selectedWarehouse}
                                onChange={(e) => setSelectedWarehouse(e.target.value)}
                                className="max-w-md px-4 py-3 rounded-xl bg-surface border border-line focus:ring-2 ring-brand-500/20 outline-none"
                            >
