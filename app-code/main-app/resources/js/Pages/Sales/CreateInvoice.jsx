...
const CreateInvoice = ({ sale }) => {
    ...
    useEffect(() => {
        if (!isEditMode && !currentInvoice) {
            // No active invoice, create one with defaults
            addInvoice({
    ...
    const handleSave = async (shouldPrint = false) => {
        ...
        const total = grandTotal; // from scope
        const excess = paid - total;

        // If excess > 0 and NOT showing modal yet, and customer is registered (not generic walk-in if names dictate)
        // Adjust logic: Only prompt if we haven't decided yet. 
        // We can pass a 'depositDecision' arg if recursively called, or use a separate state flow.
        // Simpler: If excess > 0, stop and show modal. Modal 'Confirm' calls submit with 'deposit: true/false'.

        // Check if we need to ask about deposit
        // We skip this check if the function is called FROM the modal (we'll implement a separate submit function or arg)
        // But here, let's separate the "Pre-Check" from the "Actual Submit"
        // actually, let's do it inside this function with a flag.
    };
    ...
    const processSale = async (addToLedger = false, shouldPrint = false) => {
        ...
        if (!allowOverselling) {
            for (const item of validItems) {
                ...
                // Adjust for Edit Mode: We effectively "own" the units we already bought.
                // So valid stock = Current Warehouse Stock + Original Quantity on this Invoice.
        ...
        try {
            const payload = {
                ...
                payment_account_id: currentInvoice.paymentAccountId || 1, // Default to 1 (Cash)
    ...
    const initiateSave = (print = false) => {
        ...
        if (excess > 0) {
            ...
            if (isEditMode && !editState?.overpaymentAction) {
                ...
                // Fallthrough to Modal for NEW overpayments
            }
            ...
            // Let's use a temp state for print intent
    ...
    return (
        <OneGlanceLayout title={isEditMode ? `Edit Sale #${editState?.invoiceNumber || ''}` : "Add Sale"} activeMenu="Sales" fullScreen={false} hideHeader={true} noPadding={true}>
            ...
            {aiPrefillNotice && (
                <div className="mx-4 mt-3 px-5 py-3.5 rounded-2xl bg-brand-500/10 border border-brand-500/25 flex flex-wrap items-center gap-x-3 gap-y-1">
                    <span className="text-xs font-bold uppercase tracking-wider text-brand-500">From AI Scan</span>
                    ...
                    <button
                        ...
                        className="ml-auto text-2xs font-bold uppercase tracking-wider text-ink-muted hover:text-ink-secondary dark:hover:text-white"
                    >
                        Dismiss
                    </button>
                </div>
...
`}
                        ...
                        <div className="hidden lg:flex px-3 py-2 border-b border-line items-center gap-3 bg-sunken/50 dark:bg-surface shrink-0">
                            ...
                            <div className="flex items-center justify-between w-full relative">
                                ...
                                >
                                    ...
                                    <span className="truncate">
                                        {currentInvoice.customer?.name || `Sale #${activeInvoices.findIndex(inv => inv.id === currentInvoice.id) + 1}`}
                                    </span>
                                    <ChevronDown size={11} className="text-brand-400 shrink-0" />
                                </button>
                            ...
                            <div className="flex items-center gap-1.5 w-full">
                                ...
                                <div className="flex items-center gap-1 shrink-0">
                                    <div className="flex items-center gap-0.5 bg-neutral-800 rounded-lg p-0.5 border border-neutral-700 h-[36px]">
                                        ...
                                        <button
                                            onClick={() => patchInvoice({ paymentMethod: 'cash' })}
                                            className={`px-2 py-1 rounded text-2xs font-bold transition-all ${currentInvoice.paymentMethod === 'cash' ? 'bg-orange-600 text-white shadow-sm' : 'text-ink-muted'}`}
                                        >
                                            CASH
                                        </button>
                                    </div>
                                    ...
                                    <div className="relative group/accounts-mobile shrink-0 h-[36px]">
                                        <button className="flex items-center justify-center w-9 h-[36px] rounded-lg bg-neutral-800 text-brand-400 border border-neutral-700 shadow-sm active:scale-95">
                                            <Wallet size={13} />
                                        </button>
                                                ...
                                                <div className="max-h-32 overflow-y-auto custom-scrollbar p-0.5">
                                                    {accounts.map(acc => (
                                                        <button
                                                            ...
                                                            className={`w-full text-left px-1.5 py-0.5 rounded text-3xs font-bold transition-colors flex items-center justify-between ${(currentInvoice.paymentAccountId || 1) === acc.id ? 'bg-brand-900/20 text-brand-455' : 'text-neutral-300 hover:bg-interactive-hover'}`}
                                                        >
                            ...
                            <table className="hidden md:table w-full border-separate border-spacing-y-1.5">
                                ...
                                <tbody>
                                    {showQuickEntry && !isPosted && (
                                        <tr className="bg-gradient-to-r from-brand-50/50 to-purple-50/50 dark:from-brand-900/10 dark:to-purple-900/10 border border-brand-200 dark:border-brand-800/50 rounded-xl overflow-hidden">
                                    ...
                                    {currentInvoice.items.map((item, idx) => (
                                        ...
                                        >
                                            <td
                                                className={`bg-app rounded-l-xl py-3 pl-2 ${isPosted ? 'opacity-30 cursor-default' : 'cursor-ns-resize group-active:cursor-grabbing'}`}
                                            ...
                                            <td className="bg-app py-3 relative px-2">
                                                <div className="relative">
                                                    <AsyncProductCombobox
                                            ...
                                            <td className="bg-app py-3 text-center align-middle">
                                                <div className="relative flex flex-col items-center">
                                                    <WheelInput
                                                        ...
                                                        disabled={isPosted}
                                                        className="w-16 bg-sunken border border-line dark:border-line rounded-lg text-center text-sm font-bold py-2 focus:ring-2 ring-brand-500/20 transition-all no-spinner disabled:opacity-60 disabled:cursor-not-allowed"
                                                    />
                                                    {item.product && (
                                            ...
                                            <td className="bg-app py-3 text-right align-middle">
                                                <WheelInput
                                                    ...
                                                    className="w-24 bg-sunken border border-line dark:border-line rounded-lg text-right text-sm font-bold py-2 px-3 focus:ring-2 ring-brand-500/20 transition-all no-spinner disabled:opacity-60 disabled:cursor-not-allowed"
                                                />
                                            </td>
                                            {settings?.billing_type !== 'lite' && (
                                                <td className="bg-app py-3 text-right align-middle">
                                                    <div className="flex items-center justify-end gap-2">
                                                        <WheelInput
                                                            ...
                                                            disabled={isPosted}
                                                            className="w-20 bg-sunken border border-line dark:border-line rounded-lg text-right text-sm font-bold py-2 px-3 focus:ring-2 ring-brand-500/20 transition-all no-spinner disabled:opacity-60 disabled:cursor-not-allowed"
                                                        />
                                                        <button
                                                            onClick={() => !isPosted && updateItem(item.id, 'discountType', item.discountType === 'fixed' ? 'percent' : 'fixed')}
                                                            disabled={isPosted}
                                                            className={`w-8 h-8 rounded-lg text-xs font-bold transition-all disabled:opacity-60 disabled:cursor-not-allowed ${item.discountType === 'percent' ? 'bg-brand-600 text-white' : 'bg-sunken text-ink-secondary'}`}
                                                        >
                                            ...
                                            <td className="bg-app py-3 pr-3 align-middle">
                                                <div className="flex items-center justify-end gap-1.5">
                                                    <button
                                                        ...
                                                        disabled={isPosted}
                                                        className={`w-7 h-7 rounded-md text-2xs font-bold transition-all shrink-0 border flex items-center justify-center disabled:opacity-60 disabled:cursor-not-allowed ${
                                                    ...
                                                    <WheelInput
                                                        ...
                                                        disabled={isPosted}
                                                        className="w-24 bg-sunken border border-brand-300 dark:border-brand-600 rounded-lg text-right text-sm font-bold py-2 px-3 focus:ring-2 ring-brand-500/30 transition-all text-ink no-spinner disabled:opacity-60 disabled:cursor-not-allowed"
                                                    />
                                                </div>
                                             </td>
                            ...
                            <div className="md:hidden flex flex-col gap-2">
                                    ...
                                    <div className="bg-brand-50/30 dark:bg-brand-900/10 p-3 rounded-xl border border-brand-200/50 dark:border-brand-800/50 flex flex-col gap-2 animate-in slide-in-from-top-2 duration-normal">
                                        ...
                                        <div className="grid grid-cols-3 gap-2">
                                            ...
                                            <div className="flex items-end">
                                                <button
                                                    onClick={addQuickItem}
                                                    className="w-full h-[32px] bg-brand-600 text-white rounded-lg hover:bg-brand-700 transition-all flex items-center justify-center gap-1 text-xs font-bold active:scale-95"
                                                >
                                                    <Plus size={14} /> Add
                                                </button>
                                            </div>
                                ...
                                {currentInvoice.items.length === 0 ? (
                                    <div className="bg-surface rounded-xl p-8 text-center border border-line">
                                        ...
                                        <p className="text-xs font-bold text-ink-secondary">No items added to invoice</p>
                                    </div>
                                ) : (
                                    currentInvoice.items.map((item, idx) => (
                                                ...
                                                <div className="grid grid-cols-12 gap-1.5 mt-1 items-end">
                                                    ...
                                                    <div className="col-span-3 flex flex-col gap-0.5">
                                                        ...
                                                        <WheelInput
                                                            ...
                                                            value={item.quantity ?? 1}
                                                            onChange={(e) => updateItem(item.id, 'quantity', parseFloat(e.target.value) || 0)}
                                                            className="w-full bg-app border border-line rounded-lg text-center text-xs font-bold py-1 focus:ring-1 ring-brand-500/20 outline-none"
                                                        />
                                                    </div>
                                                    ...
                                                    <div className="col-span-3 flex flex-col gap-0.5">
                                                        ...
                                                        <WheelInput
                                                            ...
                                                            value={item.price ?? 0}
                                                            onChange={(e) => updateItem(item.id, 'price', parseFloat(e.target.value) || 0)}
                                                            className="w-full bg-app border border-line rounded-lg text-right text-xs font-bold py-1 px-1.5 focus:ring-1 ring-brand-500/20 outline-none"
                                                        />
                                                    </div>
                                                    ...
                                                    <div className={`${settings?.billing_type !== 'lite' ? 'col-span-3' : 'col-span-6'} flex flex-col gap-0.5 text-right`}>
                                                        ...
                                                        <div className="flex items-center gap-1">
                                                            ...
                                                            <WheelInput
                                                                ...
                                                                value={parseFloat(calculateLineTotal(item).toFixed(2))}
                                                                onChange={(e) => handleTotalChange(item, e.target.value)}
                                                                className="w-full bg-app border border-line rounded-lg text-right text-xs font-bold py-1 px-1 focus:ring-1 ring-brand-500/20 text-ink outline-none"
                                                            />
                                                        </div>
                                                    </div>
                                                </div>
                        ...
                        <div className="lg:hidden flex flex-col shrink-0">
                            ...
                            <div className={`grid gap-1.5 px-3 py-1.5 bg-app border-t border-line shrink-0 ${
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        disabled={isPosted}
                                        className="w-full bg-sunken border border-line rounded-lg px-1.5 h-9 text-ink text-xs font-bold text-right outline-none disabled:opacity-60 disabled:cursor-not-allowed"
                                        placeholder="0"
                                    />
                                </div>
                                {showDeliveryCharges && (
                                    <div>
                                        <span className="text-4xs text-ink-muted font-bold block mb-0.5 uppercase">Delivery</span>
                                        <input
                                            type="number"
                                ...
                                {showExtraField && (
                                    <div>
                                        <span className="text-4xs text-ink-muted font-bold block mb-0.5 uppercase">Extra</span>
                                        <input
                                            type="number"
                            ...
                            {isPosted ? (
                                <div className="flex flex-col gap-1 px-2 py-2 bg-surface border-t border-line shrink-0">
                                    <div className="flex items-center gap-2">
                                        <button
                                            onClick={() => router.visit(route('store.sales.index', { store_slug: store?.slug }))}
                                            className="w-1/2 py-3 border border-line text-ink-secondary rounded-xl font-bold text-sm hover:bg-interactive-hover dark:hover:bg-interactive-hover active:scale-95 transition-all text-center flex items-center justify-center gap-1.5"
                                        >
                                            <X size={15} /> Cancel
                                        </button>
                                        <button
                                            onClick={() => initiateSave(true)}
                                            className="w-1/2 py-3 bg-brand-600 hover:bg-brand-700 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-1.5 transition-all shadow-lg active:scale-95"
                                        >
                                            <Printer size={15} /> Print
                                        </button>
                                    </div>
                                    <p className="text-center text-2xs font-bold text-red-500 uppercase tracking-wide">Posted sales can't be modified</p>
                                </div>
                            ) : (
                    ...
                    <div className="hidden lg:flex w-full lg:w-80 bg-void-700 flex-col overflow-hidden rounded-2xl shadow-2xl border border-neutral-800 shrink-0">
                        ...
                        <div className="flex-1 p-3 space-y-3 overflow-y-auto hide-scrollbar">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {settings?.invoice_number_enabled !== '0' && (
                                    <div>
                            ...
                            <div className="flex items-center justify-between bg-neutral-800/30 rounded-xl p-3 border border-neutral-700/50">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => patchInvoice({ discount: parseFloat(e.target.value) || 0 })}
                                        disabled={isPosted}
                                        className="w-20 bg-sunken/50 border border-neutral-600/50 rounded-lg px-2 py-1.5 text-white font-bold text-sm text-right focus:ring-2 ring-brand-500/20 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
                                        placeholder="0"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between bg-neutral-800/30 rounded-xl p-3 border border-neutral-700/50">
                                ...
                                <div className="flex items-center gap-2">
                                    <select
                                        ...
                                        disabled={isPosted}
                                        className="bg-sunken/50 border border-neutral-600/50 rounded-lg px-2 py-1.5 text-white font-bold text-sm focus:ring-2 ring-brand-500/20 transition-all disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer"
                                    >
                        ...
                        <div className="p-3 bg-neutral-900 space-y-2 shrink-0 border-t border-neutral-800">
                            <div className="flex justify-between items-center">
                                ...
                                <span className="text-2xl font-bold text-white">{formatCurrency(grandTotal, store || settings)}</span>
                            </div>
                            {isPosted ? (
                                <div className="space-y-2">
                                    <div className="flex gap-2">
                                        <button
                                            onClick={() => router.visit(route('store.sales.index', { store_slug: store?.slug }))}
                                            className="flex-1 py-3 bg-neutral-700 hover:bg-interactive-hover text-neutral-200 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all active:scale-95"
                                        >
                                            <X size={16} /> CANCEL
                                        </button>
                                        <button
                                            ...
                                            className="flex-1 py-3 bg-brand-600 hover:bg-brand-700 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all shadow-lg active:scale-95"
                                        >
                                            <Printer size={16} /> PRINT
                                        </button>
                                    </div>
                                    <p className="text-center text-2xs font-bold text-red-500 uppercase tracking-wide pt-0.5">Posted sales can't be modified</p>
                                </div>
                            ) : (
                                <div className="space-y-2">
                                    <button
                ...
                <div className="flex flex-col items-center py-6 text-center">
                    ...
                    <div className="grid grid-cols-1 gap-3 w-full">
                        <button
                            ...
                            className="w-full py-4 bg-brand-600 hover:bg-brand-700 text-white rounded-2xl font-bold flex items-center justify-center gap-3 transition-all shadow-xl disabled:opacity-50"
                        >
                                        ...
                                        {currentInvoice.items.filter(item => item.product).map((item, idx) => {
                                            ...
                                            return (
                                                <tr key={item.id} className="border-b border-line hover:bg-interactive-hover dark:hover:bg-interactive-hover">
                                                    ...
                                                    <td className="py-2 text-right text-xs text-ink-muted">{getCurrencySymbol(store || settings)} {cost.toLocaleString()}</td>
                            ...
                            <div className="p-4 bg-app border-t border-line shrink-0">
                                <div className="grid grid-cols-3 gap-4">
                                    <div className="bg-surface rounded-xl p-3 border border-line">
                                        ...
                                        <p className="text-lg font-bold text-ink-secondary">{getCurrencySymbol(store || settings)} {totalCost.toLocaleString()}</p>
                                    </div>
                                    <div className="bg-surface rounded-xl p-3 border border-line">
                                        <p className="text-2xs text-ink-muted font-bold uppercase mb-1">Total Revenue</p>
                                        <p className="text-lg font-bold text-ink">{formatCurrency(grandTotal, store || settings)}</p>
                                    </div>
                    ...
                    <>
                        ...
                        <div className="fixed top-0 right-0 h-full w-80 bg-surface shadow-2xl z-drawer animate-in slide-in-from-right duration-slow flex flex-col">
                            ...
                            <div className="flex-1 p-4 space-y-4 overflow-y-auto">
                                {/* Invoice Details */}
                                <div className="space-y-3">
                                    <h4 className="text-xs font-bold text-ink-muted uppercase tracking-wide">Invoice Details</h4>
                                    
                                    {/* Invoice # */}
                                    <div className="p-3 bg-app rounded-xl space-y-1.5">
                                        <p className="text-sm font-bold text-ink-secondary dark:text-white">Invoice #</p>
                                        <input
                                            type="text"
                                            value={currentInvoice.invoiceNumber || ''}
                                            onChange={(e) => patchInvoice({ invoiceNumber: e.target.value })}
                                            className="w-full bg-surface border border-line rounded-lg px-3 py-2 text-sm font-bold text-ink-secondary dark:text-white focus:ring-2 ring-brand-500/20 outline-none"
                                    ...
                                    {/* Date */}
                                    <div className="p-3 bg-app rounded-xl space-y-1.5">
                                        <p className="text-sm font-bold text-ink-secondary dark:text-white">Invoice Date</p>
                                        <input
                                            type="date"
                                            value={currentInvoice.date || ''}
                                            onChange={(e) => patchInvoice({ date: e.target.value })}
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="p-3 bg-brand-50 dark:bg-brand-900/20 rounded-xl space-y-2 border border-brand-100 dark:border-brand-800/50">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className="text-ink-muted text-xs font-bold">{getCurrencySymbol(store || settings)}</span>
                                            <input
                                                type="number"
            ...
            {showMobileSalesModal && (
                <div className="fixed inset-0 bg-neutral-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-in fade-in duration-normal">
                    <div className="bg-neutral-900 border border-neutral-800 rounded-2xl w-full max-w-sm max-h-[80vh] flex flex-col overflow-hidden shadow-2xl animate-in zoom-in-95 duration-normal">
                        {/* Header */}
                        <div className="p-4 border-b border-neutral-800 flex items-center justify-between">
                            <span className="text-sm font-bold text-white">Active Sales Sessions</span>
                            <button
                                onClick={() => setShowMobileSalesModal(false)}
                                className="p-1.5 rounded-lg bg-neutral-800 hover:bg-interactive-hover text-ink-muted transition-colors"
                            >
                                <X size={16} />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 grid grid-cols-1 gap-2.5">
                                ...
                                return (
                                    <div
                                        ...
                                        className={`p-3 rounded-xl border transition-all cursor-pointer flex items-center justify-between ${
                                            isCurrent
                                                ? 'bg-brand-950/30 border-brand-500 text-brand-400 shadow '
                                                : 'bg-neutral-800/40 border-neutral-800 hover:border-line-strong text-neutral-300'
                                        }`}
                                    >
                                        <div className="min-w-0 flex-1">
                                            <div className="flex items-center gap-2">
                                                <span className={`w-1.5 h-1.5 rounded-full ${isCurrent ? 'bg-brand-500 animate-pulse' : 'bg-neutral-600'}`}></span>
                                        ...
                                        <button
                                            ...
                                            className="p-1 rounded-md text-ink-muted hover:text-red-400 hover:bg-interactive-hover transition-colors shrink-0"
                                        >
                                            <X size={14} />
                                        </button>
                                    </div>
                            ...
                            <button
                                ...
                                className="p-3.5 rounded-xl border border-dashed border-neutral-700 text-ink-muted hover:text-white hover:border-line-strong transition-all flex items-center justify-center gap-2 text-xs font-bold bg-sunken"
                            >
