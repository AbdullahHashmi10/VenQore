...
const CreatePreSale = ({ sale }) => {
 ...
 // AsyncProductCombobox manages its own search — setting the name is sufficient
 }
 // No action needed for edit — AsyncProductCombobox refreshes on next query
 }
 } catch (error) {
 console.error(error);
 ...
 // Stock Validation REMOVED for Pre-Sales (allows negative stock)
 ...
 let response;
 if (isEditMode) {
 // Note: web.php uses sales.orders.update for pre-sales updates
...
`}
 ...
 <td className="py-2 text-right text-xs text-ink-muted">{formatCurrency(cost, store || settings)}</td>
 ...
 <div className="text-brand-400 font-bold text-xl leading-none">
