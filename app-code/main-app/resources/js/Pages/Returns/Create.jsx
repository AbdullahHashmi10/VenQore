...
const CreateReturn = ({ sale }) => {
    ...
    const handleProductSubmit = async (data, onError) => {
        try {
            ...
            if (response.data) {
                ...
                // No action needed for edit - AsyncProductCombobox handles its own state
            }
        } catch (error) {
            console.error(error);
    ...
    const processSale = async (addToLedger = false, shouldPrint = false) => {
        // Stock Validation - REMOVED for Returns (We are adding stock back)
        ...
        try {
            const payload = {
                ...
                amount_refunded: currentInvoice.amountPaid, // Reused state variable name, mapped to correct API key
            ...
            let response;
            // Always create new return (editing returns logic can be added later if needed)
            response = await axios.post(route('store.returns.store', { store_slug: store.slug }), payload);

            if (response.data.success) {
                // Global Sync Trigger (Returns affect stock)
                ...
                if (isEditMode) {
                    // Logic for edit mode if we support it later
    ...
    if (!currentInvoice || (isEditMode && !editState)) {
        return (
            <div className="flex h-screen items-center justify-center bg-app">
                <p className="text-ink-muted animate-pulse">Initializing Return...</p>
            </div>
        );
    }
...
`}
                                >
                                    ...
                                    <span className="text-xs font-bold truncate">
                                        {inv.customer?.name || `Return #${idx + 1}`}
                                    </span>
                                    <button
                                        onClick={(e) => {
                                            e.stopPropagation();
                                        ...
                                        <div className="absolute right-0 top-full pt-1 z-50 hidden group-hover/accounts-mobile:block">
                                            <div className="bg-neutral-800 rounded-lg shadow-xl border border-neutral-700 overflow-hidden w-36 p-1">
                                                <div className="p-1 border-b border-neutral-700 bg-neutral-900/50">
                                                    <p className="text-4xs font-bold text-ink-muted uppercase">Refund From</p>
                                                </div>
                                                <div className="max-h-32 overflow-y-auto custom-scrollbar p-0.5">
                                                    {accounts.map(acc => (
                                                        <button
                                                            ...
                                                            className={`w-full text-left px-1.5 py-0.5 rounded text-3xs font-bold transition-colors flex items-center justify-between ${(currentInvoice.paymentAccountId || 1) === acc.id ? 'bg-brand-900/20 text-brand-405' : 'text-neutral-300 hover:bg-interactive-hover'}`}
                                                        >
                        ...
                        <div className="hidden lg:flex px-3 py-2 border-b border-line items-center justify-between gap-3 bg-sunken/50 dark:bg-surface shrink-0">
                            ...
                            <div className="flex items-center gap-1.5">
                                <button
                                    ...
                                    className={`flex items-center justify-center w-9 h-9 rounded-xl transition-all border ${showQuickEntry ? 'bg-brand-600 text-white border-brand-500 shadow-md' : 'bg-sunken text-ink-muted border-line hover:bg-interactive-hover dark:hover:bg-interactive-hover'}`}
                                ...
                                <button
                                    ...
                                    className="hidden md:flex items-center gap-1.5 px-3 py-2 bg-sunken text-ink-secondary rounded-xl hover:bg-interactive-hover dark:hover:bg-interactive-hover transition-all border border-line shadow-sm text-xs font-bold"
                                    title="Scanning Mode"
                                >
                                    <ScanBarcode size={15} />
                                    <span>Scan</span>
                            ...
                            <div className="relative flex-1 max-w-md">
                                {currentInvoice.customer ? (
                                    <div className="relative">
                                        <div className="w-full bg-sunken border border-line rounded-xl pl-8 pr-8 py-1.5 flex items-center justify-between shadow-sm min-h-[36px]">
                                            <div className="truncate">
                                                <p className="font-bold text-ink text-2xs leading-tight truncate">{currentInvoice.customer.name}</p>
                                                <p className="text-3xs text-ink-muted leading-tight truncate">{currentInvoice.customer.phone || 'No Phone'}</p>
                                            </div>
                                            <button
                                                onClick={() => { patchInvoice({ customer: null }); setCustomerSearch(''); }}
                                                className="absolute right-2 top-1/2 -translate-y-1/2 text-ink-muted hover:text-red-500 transition-colors"
                                    ...
                                    <div className="relative">
                                        ...
                                        {/* Error Message */}
                                        {customerError && (
                                            <p className="absolute -bottom-2.5 left-3.5 bg-red-600 text-white text-4xs font-bold uppercase px-1.5 py-0.5 rounded shadow-md z-20 animate-pulse">
                            ...
                            <div className="flex items-center gap-1 shrink-0">
                                <div className="hidden md:flex items-center gap-0.5 bg-sunken rounded-xl p-0.5 border border-line">
                                    <button
                                        onClick={() => patchInvoice({ paymentMethod: 'credit' })}
                                        className={`px-2.5 py-1 rounded-lg text-2xs font-bold flex items-center gap-1 transition-all ${currentInvoice.paymentMethod === 'credit'
                                    ...
                                    <button
                                        onClick={() => patchInvoice({ paymentMethod: 'cash' })}
                                        className={`px-2.5 py-1 rounded-lg text-2xs font-bold flex items-center gap-1 transition-all ${currentInvoice.paymentMethod === 'cash'
                                ...
                                <div className="hidden md:relative md:group/accounts">
                                    <button
                                        className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-sunken text-ink-secondary hover:bg-interactive-hover dark:hover:bg-interactive-hover transition-all border border-line text-2xs font-bold min-w-[110px] justify-between"
                                    >
                                        <span className="flex items-center gap-1 truncate">
                                            <Wallet size={12} className="text-brand-500 shrink-0" />
                                            <span className="truncate">
                                                {currentInvoice.selectedBankName || accounts.find(a => a.id === (currentInvoice.paymentAccountId || 1))?.name || 'Cash in Hand'}
                                            </span>
                                        </span>
                                        <ChevronRight size={12} className="rotate-90 text-ink-muted shrink-0" />
                                    </button>
                                    ...
                                    <div className="absolute top-full pt-2 right-0 w-48 z-50 overflow-hidden hidden group-hover/accounts:block animate-in fade-in slide-in-from-top-2">
                                        <div className="bg-surface rounded-xl shadow-xl border border-line overflow-hidden">
                                            <div className="p-2 border-b border-line bg-app">
                                                <p className="text-2xs font-bold text-ink-muted uppercase">Refund From</p>
                                            </div>
                                ...
                                <button
                                    onClick={() => setShowSettingsDrawer(true)}
                                    className="flex items-center justify-center w-9 h-9 rounded-xl bg-sunken text-ink-secondary hover:bg-interactive-hover dark:hover:bg-interactive-hover border border-line shadow-sm"
                                             ...
                                             <td className="bg-app py-3 pr-3 align-middle">
                                                <div className="flex items-center justify-end gap-1.5">
                                                    <button
                                                        ...
                                                        className={`w-7 h-7 rounded-md text-2xs font-bold transition-all shrink-0 border flex items-center justify-center ${getItemTotalMode(item.id) === 'price'
                            ...
                            <div className="md:hidden flex flex-col gap-2">
                                ...
                                {currentInvoice.items.length === 0 ? (
                                    <div className="bg-surface rounded-xl p-8 text-center border border-line">
                                        ...
                                        <p className="text-xs font-bold text-ink-secondary">No items added to return</p>
                                    </div>
                                ) : (
                                    currentInvoice.items.map((item, idx) => (
                    ...
                    <div className="hidden lg:flex w-full lg:w-80 bg-void-700 flex-col overflow-hidden rounded-2xl shadow-2xl border border-neutral-800 shrink-0">
                        ...
                        <div className="flex-1 p-3 space-y-3 overflow-y-auto hide-scrollbar">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    <label className="text-3xs text-ink-muted font-bold uppercase block mb-1">Return Ref</label>
                                    <input
                                        type="text"
                            ...
                            <div className="flex items-center justify-between bg-neutral-800/30 rounded-xl p-3 border border-neutral-700/50">
                                <span className="text-xs text-ink-muted font-bold">Return Discount</span>
                                <div className="flex items-center gap-2">
                                    <span className="text-ink-muted text-xs">{getCurrencySymbol(store)}</span>
                                    <input
                                        type="number"
                                        value={currentInvoice.discount ?? 0}
                                        onChange={(e) => patchInvoice({ discount: parseFloat(e.target.value) || 0 })}
                            ...
                            {showDeliveryCharges && (
                                <div className="flex items-center justify-between p-2 hover:bg-interactive-hover rounded-lg transition-colors group">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className="text-ink-secondary text-2xs">{getCurrencySymbol(store)}</span>
                                        <input
                                            type="number"
                                            value={currentInvoice.delivery_charge ?? 0}
                                            onChange={(e) => patchInvoice({ delivery_charge: parseFloat(e.target.value) || 0 })}
                                        ...
                                        {currentInvoice.items.filter(item => item.product).map((item, idx) => {
                                            ...
                                            return (
                                                <tr key={item.id} className="border-b border-line hover:bg-interactive-hover dark:hover:bg-interactive-hover">
                                                    ...
                                                    <td className="py-2 text-right text-xs text-ink-muted">{getCurrencySymbol(store)} {cost.toLocaleString()}</td>
                            ...
                            <div className="p-4 bg-app border-t border-line shrink-0">
                                <div className="grid grid-cols-3 gap-4">
                                    <div className="bg-surface rounded-xl p-3 border border-line">
                                        ...
                                        <p className="text-lg font-bold text-ink-secondary">{getCurrencySymbol(store)} {totalCost.toLocaleString()}</p>
                                    </div>
                                    <div className="bg-surface rounded-xl p-3 border border-line">
                            ...
                            <div className="p-4 border-b border-line flex items-center justify-between bg-app">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-ink-muted">Return preferences</p>
                                    </div>
                                </div>
                                <button
                                    onClick={() => setShowSettingsDrawer(false)}
                            ...
                            <div className="flex-1 p-4 space-y-4 overflow-y-auto">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="p-3 bg-brand-50 dark:bg-brand-900/20 rounded-xl space-y-2 border border-brand-100 dark:border-brand-800/50">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className="text-ink-muted text-xs font-bold">{getCurrencySymbol(store)}</span>
                                            <input
                                                type="number"
                                    ...
                                    {/* Show/Hide Fields */}
                                    <div className="space-y-3">
                                        <h4 className="text-xs font-bold text-ink-muted uppercase tracking-wide">Return Settings</h4>
                                    ...
                                    {/* Invoice Logic */}
                                    <div className="space-y-3">
                                        <h4 className="text-xs font-bold text-ink-muted uppercase tracking-wide">Return Logic</h4>
