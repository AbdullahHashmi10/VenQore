...
const CreatePreSale = ({ sale }) => {
    ...
    // Customer search is handled by AsyncPartyCombobox
    ...
    if (!currentInvoice) return <div className="p-10 text-center text-ink-muted">Loading Workspace...</div>;
    ...
    const processSale = async (addToLedger = false, shouldPrint = false) => {
        ...
        try {
            ...
            if (response.data.success || isEditMode) { // Update might fallback
                // Global Sync Trigger (Pre-Sales don't affect stock yet, but broadcast for consistency)
    ...
    return (
        <OneGlanceLayout title={isEditMode ? `Edit Pre-Sale #${editState?.invoiceNumber || ''}` : "New Pre-Sale"} activeMenu="Sales" fullScreen={false} hideHeader={true} noPadding={true}>
            <Head title={isEditMode ? "Edit Pre-Sale" : "New Pre-Sale"} />
...
`}
                        ...
                        <div className="flex lg:hidden flex-col gap-1.5 p-1.5 bg-void-800 border-b border-neutral-800/80 shrink-0">
                            {/* Row 1: Back (Left), PreSale Pill (Center), Settings (Right) */}
                            <div className="flex items-center justify-between w-full relative">
                                ...
                                >
                                    ...
                                    <span className="truncate">
                                        {currentInvoice.customer?.name || `PreSale #${activeInvoices.findIndex(inv => inv.id === currentInvoice.id) + 1}`}
                                    </span>
                                </button>
                                                    ...
                                                    {accounts.map(acc => (
                                                        <button
                                                            ...
                                                            className={`w-full text-left px-2.5 py-1.5 rounded text-2xs font-bold transition-colors flex items-center justify-between ${(currentInvoice.paymentAccountId || 1) === acc.id
                                                                ...
                                                                : 'text-neutral-300 hover:bg-interactive-hover'
                                                                }`}
                                                        >
                        ...
                        <div className="hidden lg:flex px-3 py-2 border-b border-line items-center gap-3 bg-sunken/50 dark:bg-surface shrink-0">
                            ...
                            <div className="relative flex-1 max-w-xl">
                                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-ink-muted z-10 pointer-events-none">
                                    <User size={18} />
                                </div>
                                {currentInvoice.customer ? (
                                    <div className="relative">
                            ...
                            <div className="md:hidden flex flex-col gap-2">
                                ...
                                {currentInvoice.items.length === 0 ? (
                                    <div className="bg-surface rounded-xl p-8 text-center border border-line">
                                        ...
                                        <p className="text-xs font-bold text-ink-secondary">No items added</p>
                                    </div>
                                ) : (
                                    currentInvoice.items.map((item, idx) => (
                                        <div key={item.id} className="bg-surface p-2.5 rounded-xl border border-line shadow-sm flex flex-col gap-1.5 animate-in fade-in duration-normal">
                                            ...
                                            {item.product && (
                                                <div className="grid grid-cols-12 gap-1.5 mt-1 items-end animate-in fade-in slide-in-from-top-1 duration-normal">
                                                    {/* Qty */}
                                                    ...
                                                    <div className="col-span-2 flex flex-col gap-0.5">
                                                        ...
                                                        <div className="flex items-center gap-0.5">
                                                            <WheelInput
                                                                ...
                                                                value={item.discount ?? 0}
                                                                onChange={(e) => updateItem(item.id, 'discount', parseFloat(e.target.value) || 0)}
                                                                className="w-full bg-app border border-line rounded-lg text-right text-2xs font-bold py-1 px-0.5 focus:ring-1 ring-brand-500/20 outline-none"
                                                            />
                                                            <button
                                                                onClick={() => updateItem(item.id, 'discountType', item.discountType === 'fixed' ? 'percent' : 'fixed')}
                                                                className="px-0.5 py-1 bg-sunken border border-line rounded text-4xs font-bold shrink-0"
                                                            >
                                                                {item.discountType === 'percent' ? '%' : (getCurrencySymbol(store))}
                                                            </button>
                                                        </div>
                                                    ...
                                                    <div className="col-span-2 flex flex-col gap-0.5">
                                                        ...
                                                        <input
                                                            ...
                                                            disabled
                                                            value={parseFloat(calculateLineTotal(item).toFixed(2))}
                                                            className="w-full bg-sunken border border-line rounded-lg text-right text-2xs font-bold py-1 px-0.5 text-ink"
                                                        />
                                                    </div>
                                                </div>
                                            )}
                        ...
                        <div className="lg:hidden flex flex-col shrink-0">
                            ...
                            {/* Row 2: Cancel (25%) & Complete Pre-Sale (75%) */}
                    ...
                    <div className="hidden lg:flex w-80 bg-void-700 flex-col overflow-hidden rounded-2xl shadow-2xl border border-neutral-800 shrink-0">
                        ...
                        <div className="p-3 bg-neutral-900 space-y-2 shrink-0 border-t border-neutral-800">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <button
                                    onClick={() => initiateSave(false)}
                                    disabled={saving || isReadOnly}
                                    className="py-3 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all shadow-lg active:scale-95 disabled:opacity-50"
                                >
                                    <Save size={16} />
                                    {saving ? 'SAVING...' : 'SAVE'}
                                </button>

                                {/* CONVERT TO SALE BUTTON - Only for existing pre-sales */}
                                <button
                                    onClick={async () => {
                                    ...
                                    className={`py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all active:scale-95 ${isEditMode && !isReadOnly
                                        ? 'bg-gradient-to-r from-purple-500 to-brand-600 hover:from-purple-600 hover:to-brand-700 text-white shadow-lg '
                                ...
                                <button
                                    ...
                                    disabled={saving}
                                    className="py-3 bg-brand-600 hover:bg-brand-700 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all shadow-lg active:scale-95 disabled:opacity-50"
                                >
                                    <Printer size={16} />
                                    PRINT
                                </button>
                ...
                showProfit && !showProfitModal && (
                    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-4 duration-normal">
                        <div className="bg-neutral-900/95 backdrop-blur-lg rounded-2xl px-8 py-4 shadow-2xl border border-neutral-700 flex items-center gap-6">
                            ...
                            <p className="text-xs text-ink-muted italic">â†“ Drag down for details</p>
                        </div>
                    </div>
                )
            }
