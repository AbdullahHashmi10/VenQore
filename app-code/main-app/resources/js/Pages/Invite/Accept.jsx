        ...
        <div className="min-h-screen bg-gradient-to-br from-neutral-900 via-brand-950 to-neutral-900 flex items-center justify-center p-4 sm:p-6">
            ...
            <div className="w-full max-w-md">
                ...
                <div className="text-center mb-6 sm:mb-8">
                    <div className="w-12 h-12 sm:w-16 sm:h-16 bg-brand-600 rounded-2xl flex items-center justify-center mx-auto mb-3 sm:mb-4 shadow-2xl ">
                        <Shield className="text-white w-6 h-6 sm:w-8 sm:h-8" />
                    </div>
                    <h1 className="text-2xl font-bold text-white">VenQore</h1>
                </div>
                ...
                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-5 sm:p-8 shadow-2xl">
                    ...
                    <p className="text-ink-muted text-sm mb-6 sm:mb-8">
                        <strong className="text-brand-400">{admin_name}</strong> has invited you to join their store on VenQore.
                    </p>

                    {/* Store Info */}
                    <div className="flex items-center gap-3 sm:gap-4 p-3.5 sm:p-4 bg-white/5 rounded-2xl border border-white/10 mb-3 sm:mb-4">
                        <div className="w-10 h-10 sm:w-12 sm:h-12 bg-brand-600/30 rounded-xl flex items-center justify-center shrink-0">
                            <Building2 className="text-brand-400 w-5 h-5 sm:w-[22px] sm:h-[22px]" />
                        </div>
                        <div>
                            <p className="text-2xs sm:text-xs text-ink-muted uppercase tracking-wider">Store</p>
                    ...
                    <div className="flex items-center gap-3 sm:gap-4 p-3.5 sm:p-4 bg-white/5 rounded-2xl border border-white/10 mb-3 sm:mb-4">
                        ...
                        <div>
                            <p className="text-2xs sm:text-xs text-ink-muted uppercase tracking-wider">Invited As</p>
                            <p className="text-sm sm:text-base font-bold text-white leading-tight">{invitation?.invitee_name}</p>
                            <p className="text-xs text-ink-muted/90 leading-tight mt-0.5">{invitation?.invitee_email}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3 sm:gap-4 p-3.5 sm:p-4 bg-white/5 rounded-2xl border border-white/10 mb-6 sm:mb-8">
                        ...
                        <div>
                            <p className="text-2xs sm:text-xs text-ink-muted uppercase tracking-wider mb-2">Your Role(s)</p>
                            <div className="flex flex-wrap gap-1.5 sm:gap-2">
                                {roles.map(r => (
                                    <span key={r} className="px-2.5 py-0.5 sm:px-3 sm:py-1 bg-brand-600/40 text-brand-200 rounded-full text-2xs sm:text-xs font-bold border border-brand-500/30">
                                        {roleLabels[r] || r}
                                    </span>
                                ))}
                            </div>
                    ...
                    {/* Note */}
                    <p className="text-xs text-ink-muted mb-5 sm:mb-6 text-center leading-relaxed">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        <button onClick={accept} disabled={processing}
                            className="w-full sm:flex-1 order-1 sm:order-2 flex items-center justify-center gap-2 py-3.5 bg-brand-600 hover:bg-brand-700 text-white rounded-2xl font-bold text-sm shadow-xl transition-all active:scale-95">
                            <CheckCircle size={18} />
                            {processing ? 'Accepting...' : 'Accept Invite'}
                        </button>
                        <button onClick={decline} disabled={processing}
                            className="w-full sm:flex-1 order-2 sm:order-1 flex items-center justify-center gap-2 py-3.5 bg-white/5 hover:bg-red-500/20 border border-white/10 hover:border-red-500/30 text-ink-faint hover:text-red-300 rounded-2xl font-bold text-sm transition-all">
